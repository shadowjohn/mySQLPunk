...
- Linux / macOS 目前是跨平台預覽版，涵蓋 MySQL / MariaDB、PostgreSQL、SQL Server、SQLite 的連線、metadata、SQL 工作流程、CSV / TSV / JSON 結果匯出與常用 scalar 欄位的 Table 資料編輯；Oracle、MongoDB、Redis、Snowflake、進階型別／大型資料分頁編輯、模型與 AI 等功能仍需從 Windows 完整版遷移。Linux 缺少 `secret-tool`／Secret Service、macOS Keychain 不可用，或使用者未勾選保存時，程式重開後仍需重新輸入密碼；Linux / macOS 已可手動檢查並開啟正確平台下載頁，但背景下載、安全套用與 rollback 尚未接入，macOS 正式 Developer ID 簽署與 Apple notarization 也仍需發版憑證。
