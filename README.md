...
- **Oracle Table Designer 對權限與物件狀態較敏感 ✅ 預覽提示與權限診斷 SQL 已補齊**
  - 現況：已有多種診斷提示，例如權限不足、物件不存在、跨 schema 權限、語法不符。
  - 完成內容：Oracle SQL 預覽會在可執行 SQL 前加入註解提示，標示目標物件、直接授權需求（ALTER / CREATE INDEX / DROP / COMMENT 等）與分段執行方式；預覽註解也會附上 `all_tab_privs` 與 `session_privs` 權限診斷 SQL，方便使用者在套用 DDL 前先確認目前帳號對目標物件的直接授權與系統權限；儲存判斷也改成檢查是否存在真正可執行 SQL，避免預覽註解誤擋操作。
  - 後續方向：若需要更完整的 Oracle 風險控管，可再加入依 SQL 變更類型產生逐步確認清單。
