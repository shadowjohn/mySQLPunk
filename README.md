...
- **查詢結果匯出格式 ✅ 已補齊常用格式**
  ...
  - 完成內容：各格式會共用結果表格顯示值轉換；BLOB/geometry 會沿用結果表格的 `[Geometry] WKT` 或 `[BLOB n bytes]` 顯示，日期與空值也會一致處理；XLSX 匯出會凍結標題列、套用自動篩選、穩定欄寬與表頭樣式，讓大型 workbook 開啟後可直接篩選與辨識欄位；匯出完成後會顯示摘要視窗，列出格式、筆數、檔案大小與路徑，並可直接開啟檔案或所在資料夾。
