...
目前發版版本：`v1.0.0.22`，最新版請看 [GitHub Releases](https://github.com/shadowjohn/mySQLPunk/releases)。

目前的 `v1.0.0.22` GitHub Release 同時提供 Windows x64 完整版 setup、self-contained Linux x64／ARM64 安裝壓縮檔，以及 macOS Intel／Apple Silicon `.app.zip`。Linux 壓縮檔內附使用者層級的 `install.sh`／`uninstall.sh`；macOS 預覽目前採 ad-hoc 簽署、尚未 Apple notarize。每個跨平台資產都有獨立 `.sha256` 可驗證完整性。完整變更請見 `CHANGELOG.md`。
...
查詢結果網格可點欄名在本機依原始型別排序；NULL、跨 CLR 數字、日期、文字與 binary 都有明確比較規則，顯示用的 `(NULL)` 不會再拿去跟數字／日期比較或與同名文字混淆。相同值維持查詢回傳順序。CSV／TSV／JSON 完整匯出與複製都會沿用目前畫面順序。可用 Ctrl/Cmd+A 或延伸選取多列，再按「複製選取列」或 `Ctrl/Cmd+C` 複製為含欄名 TSV；NULL 與空字串可區分，日期、數字、binary 與 tab／換行會沿用安全匯出格式，試算表公式注入也會中和。單次限制 4 MiB，只交給系統剪貼簿，不建立磁碟暫存檔。
...
若已安裝 Docker，可再跑 MySQL 8 / MariaDB 11.4 / PostgreSQL 16 / SQL Server 2022 / TiDB 8.5 實機 round-trip；腳本只會建立並清理 `mysqlpunk-cross-*-test` 測試容器與測試自行建立的暫存 database。矩陣同時會匯出 MySQL 容器自動產生的 CA、另起一個以臨時 CA 簽發（SAN 127.0.0.1）伺服器憑證的 PostgreSQL TLS 容器，驗證 VerifyCA／VerifyFull／錯誤 CA 的 fail closed，再以同一張憑證起一個 `forceencryption` 的 SQL Server TLS 容器驗證伺服器憑證精確比對（Strict／TDS 8.0 需要伺服器端另行佈建憑證，測試容器不涵蓋），並在本機以非特權 sshd 建立 SSH Tunnel 實際查詢 MySQL、PostgreSQL 與 SQL Server。可設定 `MYSQLPUNK_TEST_FILTER=TiDB` 之類的文字只跑名稱相符的測試，`MYSQLPUNK_LIVE_OCEANBASE=1` 另外啟動 OceanBase CE（MySQL 模式，約需 3 GB 記憶體）：
...
```bash
./scripts/package-cross-platform.sh --version 1.0.0.22 --runtime linux-x64
./scripts/package-cross-platform.sh --version 1.0.0.22 --runtime linux-arm64
# 以下兩個命令需在 macOS 執行，才能驗證 codesign 與 .app archive metadata：
./scripts/package-cross-platform.sh --version 1.0.0.22 --runtime osx-x64
./scripts/package-cross-platform.sh --version 1.0.0.22 --runtime osx-arm64
```
...
```powershell
.\scripts\package-release.ps1 -Version 1.0.0.22
```
...
Windows CI 也會在一次性的 GitHub runner 安裝公開的 v1.0.0.21，再用本次建置的更新腳本實際安裝新版、確認自動重開與程式雜湊，最後解除安裝。測試只建立舊版的應用程式選項、主題與語言檔，核對新版啟動後帶入設定、保留舊檔及測試 SQL。每次的 JSON 和 log 保留在 `windows-update-install-<commit>` artifact；此驗收不操作更新按鈕，CLI 路徑等其他設定檔的跨版本搬移也還未涵蓋。
...
| 應用程式更新 | 可用 | Windows 提供單一 setup EXE；說明選單可手動檢查，也可在啟動時背景檢查。下載須有有效的 SHA-256，通過校驗才取代既有下載檔；取消或失敗會清除暫存檔。舊版 Windows x64 portable ZIP 仍可更新，不會誤選 macOS ZIP。 |
