...
- **更新、註冊、連線能力與串流匯出錯誤 ✅ 已語系化**
  ...
  - 完成內容：GitHub owner / repository、Release JSON、下載資料夾、檔案路徑、應用程式路徑、proxy 連線測試失敗、串流匯出格式與 provider 不支援等錯誤已改用語系字串；若 proxy 測試例外沒有明確原因，會依目前語系回退為「未知錯誤 / Unknown error」，繁中與英文介面都會顯示一致文字。
