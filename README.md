...
| Table Designer | 部分可用 | 既有 table ALTER 已支援 MySQL、PostgreSQL、SQL Server、Oracle；SQLite 已支援以安全重建流程處理欄位新增、刪除、重新命名、排序、型別、NULL 與 DEFAULT 變更 |
...
3. 評估是否為異種資料庫 View 複製建立 SQL 方言轉換器。
4. 補強 Oracle Table Designer 與權限不足時的錯誤訊息。
