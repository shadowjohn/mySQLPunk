...
- **由主機節點新增/刪除資料庫只支援部分 provider ✅ 高風險刪除備份、壓縮封存、還原入口與保留上限已補齊**
  ...
  - 完成內容：還原摘要已包含物件數、資料列數、欄位 schema 差異；小型資料表（目前最多 1000 列）會在還原前後建立順序無關的 SHA-256 內容指紋，即使列數不變但資料值被改動，也會在「資料內容差異」中標示變更的資料表。
  - 後續方向：若需要更完整的資料保護，可再加入大型資料表的分批/串流內容指紋，避免為超大資料集一次載入記憶體。
