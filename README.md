...
查詢結果網格可用 Ctrl/Cmd+A 或延伸選取多列，再按「複製選取列」或 `Ctrl/Cmd+C` 複製為含欄名 TSV。順序會跟目前畫面一致，NULL 與空字串可區分，日期、數字、binary 與 tab／換行會沿用安全匯出格式，試算表公式注入也會中和；單次限制 4 MiB，只交給系統剪貼簿，不建立磁碟暫存檔。
