...
- **由主機節點新增/刪除資料庫只支援部分 provider ✅ 高風險刪除備份、壓縮封存、還原入口與保留上限已補齊**
  ...
  - 完成內容：
    ...
    - 隔離區還原入口：資料庫與 Backups 節點右鍵可開啟「還原隔離備份」，從 `backup-quarantine` 選擇被隔離的備份檔，若 manifest 保留原始路徑會預填還原位置；還原動作只把備份檔移回指定位置，不會直接灌入資料庫，使用者可先檢查檔案後再走既有「還原備份」流程。
  - 後續方向：若需要更完整的資料保護，可再加入隔離備份的內容預覽與二次完整性驗證流程。
