...
目前發版版本：`v1.0.0.14`，最新版請看 [GitHub Releases](https://github.com/shadowjohn/mySQLPunk/releases)。
...
```powershell
.\scripts\package-release.ps1 -Version 1.0.0.14
```
...
#    已更新成要發布的版本，例如 1.0.0.14。
git tag v1.0.0.14
git push origin v1.0.0.14
```
...
| Punky AI 助理 | 可用 | 右側停靠聊天面板（檢視選單開啟），走 OpenAI 相容 API（OpenAI／Ollama 本機模型／自訂端點）；可附上目前資料庫結構當上下文，回覆的 SQL 一鍵插入查詢分頁；金鑰存 Windows 認證管理員。 |
