...
| Table/View 複製 | 可用 | Table 可批次複製；Oracle/PostgreSQL/SQL Server 目標建表會保留同 provider 來源 DEFAULT 與欄位註解；同 provider View 複製為 View；異種 provider View 會先嘗試轉換 SQL，無法安全轉換時改為 table snapshot |
