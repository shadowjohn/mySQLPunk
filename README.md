...
查詢結果網格可點欄名在本機依原始型別排序；NULL、跨 CLR 數字、日期、文字與 binary 都有明確比較規則，顯示用的 `(NULL)` 不會再拿去跟數字／日期比較或與同名文字混淆。相同值維持查詢回傳順序。可用 Ctrl/Cmd+A 或延伸選取多列，再按「複製選取列」或 `Ctrl/Cmd+C` 複製為含欄名 TSV；順序會跟目前畫面一致，NULL 與空字串可區分，日期、數字、binary 與 tab／換行會沿用安全匯出格式，試算表公式注入也會中和。單次限制 4 MiB，只交給系統剪貼簿，不建立磁碟暫存檔。
