...
- **結構比較：同步 SQL 預覽（Windows）**：結構差異頁新增「同步 SQL 預覽」，MySQL／MariaDB、PostgreSQL、SQL Server、SQLite 同類型資料庫之間可產生讓目標跟上來源的腳本：建立缺少的資料表（含主鍵）、新增欄位、PostgreSQL／SQL Server 的型別與 NULL 變更，以及補上缺少的外鍵（排在建表之後）。刪除只在目標的資料表、欄位、外鍵一律以註解輸出；MySQL 的 MODIFY 需要完整欄位定義，因快照不含預設值、AUTO_INCREMENT、字元集與註解，只列為附建議語法的手動項目；主鍵變更、SQLite 的欄位修改與外鍵也列為手動。Windows 版快照不含預設值、索引與外鍵規則，腳本開頭會註明。預覽視窗只提供複製與另存 .sql，不會送進查詢視窗或自動執行；跨類型比較與 Oracle 不產生腳本。smoke test 覆蓋四種 provider 的語句、註解注入與拒絕案例，產生的腳本也實際套用到 MySQL 8、PostgreSQL 16、SQL Server 2022 與 SQLite 驗證可執行。
