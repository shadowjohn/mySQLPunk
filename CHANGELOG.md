...
- 更新流程全面改為應用程式內完成：下載改附進度視窗（可取消，顯示 MB 與百分比），安裝版下載校驗後可選擇「背景靜默安裝」——mySQLPunk 自動關閉、`/VERYSILENT` 安裝完成後自動重新啟動，不再跳出安裝精靈（仍可選「否」改走手動安裝）；自我更新腳本的 PowerShell 視窗也不再閃現。安裝失敗時舊版仍在原位並會自動重啟，不會被留在沒程式可用的狀態。
