...
- **Linux / macOS 安全更新檢查**：跨平台主視窗可手動檢查最新公開 GitHub Release，依目前 OS 與 CPU 精確尋找 `linux-x64`／`linux-arm64`／`osx-x64`／`osx-arm64` 安裝包及同名 `.sha256`；解析會拒絕非 GitHub、非 HTTPS、錯誤版本與不支援架構。Linux 可在驗證後直接關閉、套用並重新啟動；macOS 在 Developer ID/notarization 完成前維持驗證後手動拖入 Applications。
