...
- **Linux／macOS 同步 SQL 預覽**：結構比較視窗新增「同步 SQL 預覽…」，同類型資料庫之間可產生讓目標結構跟上來源的指令碼：建立資料表（MySQL 取 SHOW CREATE TABLE、SQLite 取原始建立語法、PostgreSQL／SQL Server 依欄位與主鍵組出）、新增與修改欄位、建立或重建索引、新增或重建外鍵、重建檢視表，依建表→欄位→索引→外鍵→檢視表排序，外鍵延後建立避免參照表尚未存在。會刪除資料或物件的變更（只在目標的資料表、欄位、索引、外鍵、檢視表）一律以註解輸出、預設不執行；型別收窄、SQL Server 具名預設值、主鍵變更、identity／generated 差異、SQLite 無法 ALTER 的欄位與外鍵等列在「需手動處理」。視窗只提供複製與另存 .sql，不送進編輯器，因為編輯器連的是來源；跨類型比較不產生腳本。所有註解文字都壓成單行，惡意物件名稱無法逃出註解。smoke test 與 Docker 實機矩陣在 SQLite、MySQL 8、MariaDB 11.4、PostgreSQL 16、SQL Server 2022 上執行「產生→套用→重新比較」，確認只剩被註解的破壞性差異。
