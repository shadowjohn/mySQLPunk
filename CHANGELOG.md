...
- **匯出／匯入工作區（協同合作第一階段，Windows）**：「檔案」選單新增「匯出工作區...」與「匯入工作區...」。匯出把目前設定檔的連線（含群組、星號與顏色；一律移除密碼、SSH／SSL 祕密、權杖與本機憑證參照，登入帳號預設不含）、SQL 片段、AI 動作、自動執行作業（Webhook 網址可能含權杖，預設不含）與自訂資料產生器字典寫成資料夾：JSON 鍵排序、固定順序、LF 換行、沒有時間戳記，重複匯出內容完全相同，並附 `.gitattributes`，可直接提交到 Git 與隊友共用。匯入先比較每個項目為「新增／變更／相同」並讓你勾選要套用的項目；自動執行作業取代時保留本機的 Webhook；接著開啟既有的連線匯入預覽，工作區的連線一律以合併方式匯入並沿用本機密碼，沒有帳號的連線會對應到本機相同的連線而不重複新增。
