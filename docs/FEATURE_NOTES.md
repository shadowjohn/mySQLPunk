...
- **Linux / macOS 跨平台預覽 🟡 第二階段進行中**
  ...
  - 同步 SQL 預覽：同類型資料庫的比較結果可產生讓目標跟上來源的指令碼（建表、加改欄位、索引、外鍵、檢視表），刪除類變更一律註解、無法安全表達的列為手動項目；只能複製或另存 .sql，不送進編輯器，也不自動執行。smoke test 與 Docker 實機矩陣在五種資料庫上驗證產生→套用→重比後只剩被註解的差異，並驗證惡意名稱無法逃出註解。
