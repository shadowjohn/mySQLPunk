...
- **Linux / macOS 跨平台預覽 🟡 第二階段進行中**
  ...
  - MariaDB 原生 UUID／INET6：metadata 分別映射為 Guid 與 NetworkAddress 編輯器，載入會明確 cast text，UUID 正規化為小寫 D 格式，INET6 接受 IPv4／IPv6 並將 IPv4 保存為 mapped IPv6。寫入與原值 predicate 使用文字參數及 `CAST(... AS UUID/INET6)`，錯誤 UUID、CIDR prefix 與 IPv6 zone identifier 在送出前 fail closed。MariaDB 11.4 實機矩陣涵蓋 metadata、IPv4／IPv6 canonical 化、CRUD、錯誤輸入不落資料與 optimistic conflict；CI 會與 MySQL 8、PostgreSQL 16、SQL Server 2022 同時啟動驗證。Linux X11 另從實際 Table 編輯器修改 UUID 與 IPv4，畫面重新載入 canonical UUID／mapped IPv6 後再直接查庫與底層 HEX 核對。
