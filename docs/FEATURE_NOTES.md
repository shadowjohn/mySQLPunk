...
- **Windows 更新流程**
  ...
  - 可攜包解壓後先檢查主程式數量與位置；只接受根目錄或單一包裝目錄中的一份 `mySQLPunk.exe`。缺檔、多份主程式、混雜包裝目錄或損毀 ZIP 都在複製前停止，原安裝保持不變；合法包複製途中遇到 I/O 錯誤時，目前仍沒有 rollback。
  - 首次啟動若沒有本版 `application-options.json`，會從同產品目錄選取最近舊版的選項，檢查格式後以同目錄暫存檔搬入。來源保留、新版既有設定不覆寫；讀取失敗會阻止自動存檔用預設值蓋掉原資料。這次只處理應用程式選項，theme、language、CLI 路徑等其他版本化檔案仍待搬移。
