...
這裡放的是各功能做完的細節紀錄，原本堆在 README 的「未完成功能與已知限制」，越長越大搬過來的。README 只留功能概況表跟現在還存在的限制；做完一個功能，細節寫在這裡，README 那邊把狀態改一下就好。
...
- **Windows 更新流程**
  ...
  - 可攜包解壓後先檢查主程式數量與位置；只接受根目錄或單一包裝目錄中的一份 `mySQLPunk.exe`。缺檔、多份主程式、混雜包裝目錄或損毀 ZIP 都在複製前停止。套用前只備份會覆寫的檔案，逐檔驗證後替換；遇到 I/O 錯誤會反向回復、核對原雜湊並移除本次新增的檔案及空目錄。若回復不完整，暫存目錄會保留備份與 `journal.json`，且不重啟程式。斷電或強制終止腳本後的自動復原尚未涵蓋。
  - 首次啟動若沒有本版 `application-options.json`，會從同產品目錄選取最近舊版的選項，檢查格式後以同目錄暫存檔搬入。`theme.txt` 與 `language.txt` 也會帶入舊版支援的主題及語言。來源保留、新版既有設定不覆寫；讀取失敗會阻止自動存檔用預設值蓋掉原資料。CLI 路徑等其他版本化檔案仍待搬移。
  - smoke tests 涵蓋實際 HTTP 下載取消、校驗碼缺漏／格式錯誤／不符、既有下載保留，以及產生的 PowerShell 腳本之逾時、路徑與失敗代碼。可攜版另以真實檔案鎖驗證更新中斷後的完整檔案雜湊與目錄回復，並檢查備份失敗及 junction 邊界。
  - Windows CI 另以 `Test-WindowsUpdateInstall.ps1` 在 GitHub 一次性 runner 下載並驗證公開 v1.0.0.21 安裝檔，安裝到含空白、中文與單引號的測試目錄，再由目前產生的更新腳本執行真正的安裝與重新啟動，核對版本、程式 SHA-256、測試資料保留及解除安裝結果。選項只在舊版目錄建立，驗收會確認新版自行帶入 bool／int／string 選項，且舊版關閉後的設定檔 SHA-256 保持不變。測試不替換 installer 或 `Start-Process`，JSON 與 log 會保留為 CI artifact；這是更新服務／安裝腳本的驗收，不包含點擊 UI 更新按鈕。腳本只接受 GitHub hosted Windows runner，不會在開發者電腦安裝程式。
...
- **Linux / macOS 跨平台預覽 🟡 第二階段進行中**
  ...
  - UI：連線設定新增／編輯／刪除、SQLite 檔案選擇、資料庫切換、Table / View 物件樹、選取範圍／游標 statement／整份文件的明確執行入口、取消操作、動態結果網格與狀態／錯誤顯示。`Ctrl/Cmd+Enter` 預設只執行選取範圍或游標 statement，`Ctrl/Cmd+Shift+Enter` 才執行整份；statement lexer 依 provider 略過字串、quoted identifier、註解及 PostgreSQL dollar quote／`E''` 內分號，MySQL 反斜線跳脫若受 `NO_BACKSLASH_ESCAPES` 影響而有兩種邊界則 fail closed、要求明確反白。雙擊 Table 會開啟每頁 200 列的資料編輯器，依複合 Primary Key ordinal 穩定排序並提供上一頁／下一頁、頁碼與列範圍；雙擊 View 則產生預覽 SQL。編輯器依 metadata 顯示 PK、generated、nullable 與型別，支援 Value／NULL／DEFAULT 新增、修改、刪除與重新整理；無 Primary Key 時為避免 offset 定位漂移，只顯示第一頁。
  - 安全：跨平台連線設定寫入使用者 application-data 下的 `mySQLPunk/connections.json`；Linux 在全新環境尚未建立 ApplicationData 時會遵循絕對的 `XDG_CONFIG_HOME` 或 `~/.config`，macOS 則回到 `~/Library/Application Support`，不再嘗試寫入程式安裝目錄。`Password` 明確排除序列化，只有非祕密的 `UseSecretStore` opt-in 旗標會保存，非 Windows 檔案權限會嘗試限制為 `0600`。Linux 透過 `secret-tool` 的 stdin 接 Secret Service，macOS 透過 `security -i` 的 stdin 接 Keychain，祕密不出現在 process arguments；寫入後會讀回比對，工具缺失、服務鎖定或驗證失敗時不建立檔案 fallback，只保留於本次程式記憶體。刪除連線或取消保存會同步清除 Keyring 項目。
  - 連線 URI 與 TLS 模式：連線設定頁可貼上 `mysql://`／`mariadb://`、`postgres://`／`postgresql://`、`mssql://`／`sqlserver://` 或 `sqlite:///` URI 並「安全套用」到表單，自動填入 provider、主機、port、認證、資料庫、逾時與 TLS 模式，不自動連線或儲存；輸入框遮蔽顯示且成功後即清除。TLS 改為 provider 原生模式下拉（MySQL Disabled／Preferred／Required／VerifyCA／VerifyFull、PostgreSQL 另含 Allow、SQL Server Optional／Mandatory／Strict），未指定時保留驅動程式預設而非降級；`connections.json` 以 `tlsMode` 保存，舊 `useSsl` 依 provider 遷移為等價模式，同時含兩者、含 `password`、重複欄位、非法模式或空白／重複 id 一律 fail closed。URI 解析拒絕未知／重複參數、fragment、多層或 dot-segment 資料庫路徑、未編碼空白、非法 percent／UTF-8、Unicode 控制／格式／noncharacter、過長內容與 provider 不相容的 TLS 值，錯誤訊息不回顯密碼；smoke test 覆蓋四種 provider 的成功案例、IPv6、前後空白密碼、SQLite 尾端空白路徑、legacy 遷移與三十餘個 fail-closed 案例。操作方式見 [`CONNECTION_SECURITY.md`](CONNECTION_SECURITY.md)。
  - TLS 憑證檔案：連線設定頁可指定 CA 憑證（SQL Server 為伺服器憑證）、客戶端憑證與私鑰的絕對路徑，或由 URI 的 `sslca`／`sslrootcert`／`sslcert`／`sslkey`／`servercertificate` 帶入，對應 MySqlConnector、Npgsql 與 SqlClient 原生選項。CA 憑證只接受 VerifyCA／VerifyFull（SQL Server 為 Mandatory／Strict），客戶端憑證與私鑰須成對且模式至少 Required，SQL Server／SQLite 不接受客戶端憑證；路徑須為絕對路徑且不含控制字元，儲存、測試與連線前確認檔案存在、非目錄且私鑰不可被其他使用者讀取。`connections.json` 只保存路徑；smoke test 覆蓋三種 provider 的連線字串映射、十餘個不相容組合、缺檔／目錄／權限過寬、設定檔 round-trip 與 URI 參數。規則見 [`CONNECTION_SECURITY.md`](CONNECTION_SECURITY.md)。
  - 執行計畫：查詢工具列「執行計畫」依游標 statement 或反白範圍取得 MySQL／MariaDB JSON、PostgreSQL JSON（ANALYZE FALSE）、SQL Server SHOWPLAN_ALL（同連線開關，失敗必關）與 SQLite EXPLAIN QUERY PLAN 的唯讀計畫；獨立視窗顯示樹狀節點與相對成本標示、節點屬性、文字計畫與原始資料。只接受單一 SELECT／WITH／DML，拒絕 DDL 與多 statement，解釋 INSERT／DELETE 不會修改資料；smoke test 與 Docker 實機矩陣驗證四種 provider 的解析、列數不變與 SHOWPLAN 清理。
  - 資料產生器：物件面板「資料產生器…」可勾選多張資料表並設定筆數，逐欄選擇規則（自動、資料庫預設、NULL、固定值、序列、範圍、清單、樣式與 NULL 比例），可指定 seed 重現。自動規則依型別、欄位名稱（email、姓名、電話、城市、金額、生日等）、長度與 ENUM／SET 成員產生；外鍵欄位挑選現有或同批產生的父列（含自我參照與複合外鍵），主鍵與唯一索引不與既有或同批資料重複，被參照的自動編號主鍵改寫明確值並推進序列。每個值寫入前都經資料編輯器的型別驗證，預覽只在記憶體產生，寫入在單一交易中完成、任何一列失敗就整批回滾。smoke test 與 Docker 實機矩陣在 SQLite、MySQL 8、MariaDB 11.4、PostgreSQL 16、SQL Server 2022 驗證外鍵完整性、唯一性、規則、seed 重現、並行衝突回滾與寫入後預設新增不撞主鍵。
  - 資料比較與同步：結構比較視窗的「比較資料…」以 Primary Key 逐列比對同名資料表（每表上限 50,000 列），可預覽 SQL 並在目標以單一交易同步；依外鍵相依排序、每列以比對當下的目標原值為條件（比對後被修改則整批回滾），刪除預設不勾且需輸入資料庫名稱，identity 主鍵保留來源值並重設 PostgreSQL 序列，寫入沿用資料編輯器的型別轉換。五種資料庫的實機往返與並行回滾皆驗證。
  - 同步 SQL 預覽：同類型資料庫的比較結果可產生讓目標跟上來源的指令碼（建表、加改欄位、索引、外鍵、檢視表），刪除類變更一律註解、無法安全表達的列為手動項目；可複製或另存 .sql，也可在「在目標執行」分頁勾選後於目標連線執行（刪除類預設不勾且需輸入資料庫名稱確認；PostgreSQL／SQL Server／SQLite 單一交易、失敗全部回滾，MySQL／MariaDB 遇錯即停並標示已生效語句），不送進編輯器、完成後自動重比。smoke test 與 Docker 實機矩陣在五種資料庫上驗證產生→套用→重比後只剩被註解的差異，並驗證惡意名稱無法逃出註解。
  - 結構比較：物件面板「結構比較…」比對目前資料庫與任一已儲存連線的資料庫（可跨 provider），列出物件、欄位、索引、外鍵的只在來源／只在目標／內容不同，可篩選並匯出 HTML；主鍵依角色、外鍵依欄位清單比對，型別大小寫與 SQL Server 預設值括號正規化，無法讀取的物件只列警告。只讀 metadata、不產生同步 SQL；smoke test 覆蓋 SQLite 雙庫差異、跨 provider 名稱對應與正規化、無法讀取物件。
  - 資料字典：物件面板「資料字典…」把目前資料庫所有資料表／檢視表匯出成單一 HTML（欄位、索引、外鍵、註解、建立語法或檢視表定義），四種 provider 只讀目錄 metadata；無權限的物件在文件內註明，所有文字 HTML 逃逸並附 CSP，錨點雜湊、64 MiB 上限、暫存檔原子替換。smoke test 與 Docker 實機矩陣覆蓋四種 provider 的結構欄位。
  - SSH Tunnel：連線設定可勾選透過 SSH Tunnel 連線，填入 SSH 主機／port／使用者、密碼或 OpenSSH／PEM 私鑰（含密語）與必填的主機金鑰 SHA256 指紋；以 SSH.NET 建立只綁定 127.0.0.1 動態 port 的本機轉送，MySQL／MariaDB、PostgreSQL、SQL Server 的每個 session 共用一條 SSH 連線，中斷連線或關閉程式即釋放，跳板機斷線後下一次操作會自動重建。主機金鑰完全比對、不符即中止並回報實際指紋；MD5 指紋、缺主機／使用者／指紋、非絕對路徑或權限過寬的私鑰、MySQL／PostgreSQL 的 VerifyFull 一律 fail closed，SQL Server 改以原始主機名稱驗證憑證。SSH 密碼與密語不寫入設定檔，勾選系統密碼庫時以獨立項目存入 Secret Service／Keychain，否則只保留到本次程式關閉；smoke test 用本機臨時 sshd 實機驗證金鑰登入、加密私鑰、錯誤密語、指紋不符、權限過寬與 provider session 整合。
  - 結果排序、複製與匯出：動態欄位使用原始 provider 值的型別感知 comparer；NULL 遞增時置前、跨 CLR 數字先正規比較、日期與文字保留各自語意、binary 使用 byte lexicographic 順序，無法直接比較的異質值仍有不拋例外的穩定 fallback，相同值保留查詢回傳順序。顯示用 `(NULL)` 與資料中的同名 literal 維持不同值。結果網格可延伸選取多列，以按鈕或 `Ctrl/Cmd+C` 依目前可視順序複製含欄名 TSV；流程使用原始結果值而非畫面 `(NULL)`，區分 NULL／空字串、保留 invariant 日期／數字／binary，沿用公式注入中和並限制為 4 MiB，只寫系統剪貼簿。完整結果仍可明確選擇 CSV、TSV 或 JSON，再由 Linux / macOS 原生儲存對話框選擇檔案；三種格式都會依目前結果網格的排序輸出完整載入列。CSV / TSV 採 UTF-8 BOM 與 CRLF，正確處理分隔符、引號、換行、空字串、NULL、日期、數字與 binary hex；JSON 不帶 BOM，保留 JSON 可表達的型別並為重複欄名建立穩定唯一名稱。輸出先寫同目錄暫存檔，成功後才替換目標；截斷結果會在匯出前提示只包含目前載入的前 10,000 列。
  - 安全資料編輯：新增、修改與刪除只接受 schema metadata 中存在且可編輯的欄位，所有值使用 provider 參數，不把輸入拼入 SQL。修改／刪除要求 Primary Key，WHERE 同時比對 PK 與載入時的可編輯原始值；影響 0 列視為外部變更衝突，超過 1 列也會回復交易，避免靜默覆蓋或批次誤改。MySQL／MariaDB BLOB、PostgreSQL bytea、SQL Server binary／varbinary／image 與 SQLite BLOB 接受 `0x` 加偶數個 hex 字元並以原生 binary 參數寫入，上限 1 MiB；MySQL／MariaDB BIT(1–64) 接受非負十進位整數，會依欄位寬度拒絕溢位並以 MySqlDbType.Bit 寫入；ENUM／SET 會保留 COLUMN_TYPE 宣告並使用 MySqlDbType.Enum／Set，SET 由 server 依宣告順序正規化，strict mode 拒絕未宣告值時回復交易；TIME 使用專用值型別，不受 `TimeSpan.Parse` 的 24 小時格式限制，可依 TIME(0–6) 精度解析正負 `HHH:mm:ss.ffffff`，上限為 ±838:59:59 且以 MySqlDbType.Time 寫入，載入則 cast text 保留 MySQL 原生顯示。PostgreSQL bit(n)／bit varying(n) 使用純 0／1 字串，metadata 保留欄位長度並驗證精確／最大 bit 數，再轉成 Npgsql BitArray 參數；載入時明確 cast text 以保留前導零。PostgreSQL time with time zone 使用 `HH:mm:ss.ffffff±HH:mm`，要求明確 offset 並以 Npgsql TimeTz 寫入；載入時 cast text，原值 predicate 再嚴格轉回 DateTimeOffset。PostgreSQL interval 使用 `months=<整數>;days=<整數>;microseconds=<整數>` 保留原生三分量並以 NpgsqlInterval 寫入；載入與原值 predicate 也按三分量正規化，不用 `TimeSpan` 近似月數，且不會把 `1 month` 與 `30 days` 視為同一原值；metadata 另保留 `interval(p)` 與 `YEAR`、`YEAR TO MONTH`、`DAY TO HOUR`、`DAY TO SECOND(p)` 等宣告，寫入前拒絕會被 server 丟棄或取整的較小分量。PostgreSQL pg_lsn 要求 `XXXXXXXX/XXXXXXXX` 格式，斜線兩側各限制為 32-bit 十六進位值並正規化為大寫，再以 NpgsqlLogSequenceNumber／PgLsn 原生參數寫入。PostgreSQL oid／xid／cid 使用 UInt32，xid8 使用 UInt64，並繞過 Npgsql 不支援的通用 DbType.UInt64、直接指定各自原生型別，避免高位值被有號整數截斷。PostgreSQL tsvector／tsquery 禁止 NUL 且限制 1 MiB；因 Npgsql 8 client parser 已標示為不可靠且 obsolete，寫入與 optimistic 原值會以 Unknown 參數讓 PostgreSQL 在型別上下文中權威解析，仍維持 SQL 與值分離，server 拒絕畸形內容時整筆交易回復。MySQL／MariaDB JSON、PostgreSQL json／jsonb 與 SQLite JSON 會先經嚴格 JSON parser 驗證，再以 provider 專用參數寫入，並針對 MySQL 儲存正規化與 PostgreSQL json 無原生等號的差異使用正確原值 predicate。PostgreSQL xml、SQL Server xml 與 SQLite XML 會禁止 DTD／外部實體並限制文件深度，原生 provider 另使用 XML 參數與可比較的序列化原值 predicate。PostgreSQL inet／cidr／macaddr／macaddr8 會驗證 address family、prefix、CIDR host bits、zone identifier 與 MAC byte 長度，再轉成 Npgsql 原生型別；載入時由 PostgreSQL 轉成文字以保留 inet prefix。SQL Server text／ntext／image 使用對應的 SqlDbType，optimistic predicate 則轉成 varchar(max)／nvarchar(max)／varbinary(max) 後逐 byte 比對，避開 legacy LOB 不可直接使用等號的限制。超過 1 MiB 的既有 binary／JSON／XML／bit string／全文檢索值只顯示摘要，且不放入非 PK optimistic predicate。無 Primary Key 的 Table 只允許新增與瀏覽，generated 及尚未支援的進階型別維持唯讀。
  - MySQL／MariaDB ENUM：解析 `COLUMN_TYPE` 的完整宣告成員，在建立 provider 參數前以 ordinal 精確比對，拒絕 collation、尾端空白與數字索引會無 warning 改成另一成員的輸入。metadata parser 保留逗號、引號、反斜線、空字串與數字字串；MySQL 8／MariaDB 11.4 實機矩陣先重現四種原生靜默正規化，再驗證全部特殊成員 round-trip 與錯誤輸入不落資料。Linux X11 另確認 `DRAFT` 留在編輯視窗且資料庫原值不變，精確的 `published` 則可保存並刷新表格。
  - MySQL／MariaDB SET：沿用宣告成員 metadata parser，將輸入拆成逗號分隔成員後逐一做 ordinal 精確比對，拒絕 collation、尾端空白與數字 bitmap 會無 warning 改變所選集合的輸入。合法任意順序與重複成員依 SET 無序集合語意輸出為宣告順序；含空字串或逗號、無法以文字無歧義 round-trip 的歷史 schema 保持唯讀。MySQL 8／MariaDB 11.4 實機矩陣涵蓋特殊成員、空集合、單一與多成員 bitmap、canonicalization 與錯誤輸入不落資料；Linux X11 另確認 `ALPHA` 被 UI 拒絕且原值不變，`Beta,alpha` 保存後刷新為 `alpha,Beta` 並直接查庫一致。
  - PostgreSQL 文字序列化型別：`jsonpath`、`pg_snapshot`／`txid_snapshot`、`hstore`、`ltree`／`lquery`／`ltxtquery`、11 種 `reg*` 物件參照，以及任意 enum／composite／extension UDT 改以最多 1 MiB 的純文字編輯；參數保持 Unknown，讓欄位型別上下文交由 PostgreSQL 權威 parser 驗證。載入一律轉 canonical text；沒有原生等號的型別以載入時文字做 optimistic predicate，仍可偵測其他連線改動。PostgreSQL 16 實機矩陣涵蓋 22 個代表型別的 metadata、新增、修改、畸形 jsonpath／composite rollback、衝突與刪除，其中自訂型別直接覆蓋 enum、named composite、`cube` extension 與 enum domain；Linux X11 另從 UI 修改 jsonpath／ltree，以及 enum／named composite／`cube` UDT 後直接查庫核對。
  - PostgreSQL domain：metadata 分開保存 schema-qualified domain 與 base storage type，例如 `"public"."short_label" (character varying(30))`；integer、varchar、numeric、enum／UDT、CIDR 等 domain 沿用 base type 的解析、參數與 optimistic predicate，並保留 domain constraint 權威驗證。PostgreSQL 16 實機矩陣涵蓋五種 base type 的 metadata、CRUD、constraint rollback、衝突與刪除；Linux X11 另從 UI 修改全部五種 domain 後直接查庫核對。
  - PostgreSQL scalar temporal：`timestamp(p) without time zone`、`timestamp(p) with time zone`、`time(p) without time zone` 與既有 `time(p) with time zone` 會依 format_type metadata 保留 0–6 位小數秒精度並使用對應的 Npgsql 原生參數。無時區 timestamp 禁止 offset；timestamptz 必須包含明確 offset 或 Z，載入固定正規化為 UTC `Z`，不受 session timezone 影響；time 保留 PostgreSQL 合法的 `24:00:00` 上界。超過欄位 p 的值會在送出前拒絕，避免 server 取整。PostgreSQL 16 實機矩陣涵蓋 metadata、0001／9999 年、毫秒／微秒、offset 轉 UTC、24:00、錯誤輸入不落資料、CRUD 與 optimistic conflict；Linux X11 另驗證 `timestamp(3)` 第 4 位小數被 UI 拒絕，再修改四欄並以 `psql` 逐欄查回。
  - PostgreSQL／SQLite 純日期：SQLite `date` 只接受 `yyyy-MM-dd`；PostgreSQL `date` 使用獨立文字 canonical 型別，接受 `YYYY-MM-DD`、`YYYY-MM-DD BC` 與 `±infinity`，載入以 `EXTRACT` 組回不受 DateStyle 或 timestamp 上限影響的 4713 BC–5874897 AD 完整範圍。參數維持 Unknown 交給 date 欄位上下文解析，避開 Npgsql 把 .NET `DateTime.MinValue`／`0001-01-01` 自動送成 `-infinity`；完整 timestamp 會在送出前拒絕。PostgreSQL 16 實機矩陣涵蓋 BC／AD 上下界、0001／9999 年、正負 infinity、NULL、CRUD、optimistic predicate 與錯誤輸入不落資料；Linux X11 另確認 grid／編輯器顯示純日期、timestamp 被 UI 拒絕，以及從 `0001-01-01` 修改後直接查庫一致。
  - SQLite NUMERIC affinity：SQLite 不強制 `DECIMAL(p,s)` 的 precision／scale，Microsoft.Data.Sqlite 的 decimal TEXT 參數仍會被 NUMERIC affinity 轉成 INTEGER 或 REAL。編輯器因此將可落在 signed 64-bit 的整數維持完整範圍，其餘數值最多接受 15 位有效數字，並拒絕會在 binary64 轉換中無聲失真的輸入；載入一律顯示 SQLite 實際儲存值的 canonical text，固定小數與科學記號可按數值等價完成 optimistic concurrency 比對。SQLite round-trip 測試涵蓋 REAL／INTEGER storage class、Int64 上界、安全科學記號、失真輸入不落資料及外部修改衝突。
  - SQLite temporal：`DATE`、`TIME`、`DATETIME`、`TIMESTAMP` 與 `DATETIMEOFFSET` 不再先轉成 .NET `DateTime`／`DateTimeOffset`；編輯器分別要求純 `yyyy-MM-dd`、`HH:mm:ss[.fffffff]`、不含 offset 的 `yyyy-MM-dd[ T]HH:mm:ss[.fffffff]`，或必須包含 `±HH:mm` 的 offset 日期時間，再以 TEXT 參數原樣保存。這避免 Date 被補午夜、Time 被注入當天日期、offset 被轉成本機時間後丟失，以及超過 7 位小數秒；offset 的原始文字也不會被正規化到 UTC。舊有非 canonical／INTEGER／REAL 值只要未修改就以原始顯示判定相同，不會在修改其他欄位時被連帶重寫。SQLite 端到端測試涵蓋五種 metadata、TEXT storage class、7 位小數秒、正負 offset、錯誤輸入不落資料與 optimistic conflict；Linux X11 另先重現 `TIME` 注入日期、`DATE` 補午夜與合法 `DATETIMEOFFSET` 被錯誤拒絕，再確認修正版拒絕缺 offset 值，合法純日期／時間／日期時間／offset 日期時間保存後以原生 client 核對文字與 hex。
  - SQLite UUID／GUID：自訂宣告不再落到 NUMERIC affinity 編輯器；兩種 metadata 都映射到 SQLite 專用 GUID 文字型別，只接受含連字號的 8-4-4-4-12 D 格式，再以 TEXT 參數原樣保存大小寫，避免 Microsoft.Data.Sqlite 的 Guid 參數自動轉成大寫。既有大括號等非標準文字與 16-byte BLOB 未修改時仍以原始 storage class 做 optimistic predicate。SQLite 端到端測試涵蓋 metadata、大小寫、TEXT／BLOB storage class、錯誤輸入不落資料、legacy 原值及外部修改衝突；Linux X11 另先重現標準 UUID 被誤判為數字，再確認無連字號值由 UI 拒絕，合法大小寫分別保存後以 `sqlite3` 核對文字與 hex。
  - SQLite byte-exact concurrency：String、JSON、XML、temporal 與 GUID 的原值 predicate 會將欄位與參數都 `CAST(... AS BLOB)` 後比較，避開 `NOCASE`、`RTRIM` 或應用程式自訂 collation 把不同 bytes 判成相等。SQLite 端到端測試先以 `Alpha→alpha` 和 `tail `→`tail` 重現 stale update 可提交，再確認修正版回復交易、保留外部值與 marker，重新整理後仍可合法修改；Linux X11 另在編輯 modal 開啟期間由第二連線改變大小寫，確認儲存顯示衝突且不覆寫其它欄位。
  - IEEE 754 浮點：依 provider metadata 分開映射 4-byte single 與 8-byte double；SQL Server `float(n)` 保留 n 並以 24 為界，MySQL／MariaDB `FLOAT(p)` 也依 24 為界，`FLOAT(M,D)` 另驗證小數位、整數位與 UNSIGNED。輸入先正規化十進位／科學記號，再要求與目標 single／double 的 `R` canonical 文字數值相同，拒絕會改變數字、溢位、subnormal、下溢為 zero、NaN 與 Infinity；參數固定使用各 provider 的 Real／Float／Double 原生型別。MySQL／MariaDB `FLOAT` 查詢會先 CAST 為 DOUBLE 取回完整 binary32 值，避免 text protocol 預設只輸出約 6 位有效數字；原值 predicate 使用同一路徑，仍能偵測外部修改。SQLite、MySQL 8、MariaDB 11.4、PostgreSQL 16 與 SQL Server 2022 測試涵蓋 metadata、single／double／precision 宣告、CRUD、降精度輸入不落資料、optimistic conflict 與刪除；Linux X11 另從實際 Table 編輯器確認 canonical REAL 顯示、UI 拒絕超精度輸入，合法修改後再由 `sqlite3` 直接查回一致。
  - Provider-aware integer range：欄位 metadata 保存實際 base integer 的 minimum／maximum；MySQL／MariaDB 涵蓋 signed／unsigned／`ZEROFILL` 的 `TINYINT`、`SMALLINT`、`MEDIUMINT`、`INT`、`BIGINT`，PostgreSQL 涵蓋 `smallint`／`integer`／`bigint`，SQL Server 另將 `tinyint` 視為 0–255。輸入在建立 provider 參數前先依邊界拒絕，PostgreSQL domain 與 SQL Server alias type 也沿用 base storage type，不再只依共用 `long`／`ulong` 上限。實機矩陣會把 MySQL／MariaDB 切到 non-strict session，確認越界值不會被 server 靜默截到最近邊界，並涵蓋所有正負上下界、`BIGINT UNSIGNED` 全範圍、CRUD、optimistic conflict 與刪除；Linux X11 另驗證 non-strict MySQL UI 拒絕 `TINYINT=128` 且資料庫原值不變，再保存合法下界 `-128` 並直接查庫一致。
  - PostgreSQL money：metadata 以資料庫當前 `lc_monetary` 實際推導固定小數位，不依賴 `information_schema` 會回傳 NULL 的 numeric scale。編輯器只接受 locale-independent 十進位文字，先依小數位與底層 signed 64-bit 儲存範圍拒絕會被 server 取整或溢位的值；載入透過 lossless `money → numeric → text` 顯示 canonical 金額，寫入使用 Unknown 文字參數明確 CAST 回 money，避免幣別符號與千分位受到 Linux／macOS locale 影響。PostgreSQL 16 實機矩陣涵蓋 metadata、正負範圍邊界、CRUD、多餘小數／溢位／locale 格式拒絕、optimistic conflict 與刪除；Linux X11 另確認 grid 不顯示 locale 幣別／千分位、UI 拒絕第 3 位小數，合法修改後再由 `psql` 直接查回一致。
  - Spatial 型別：MySQL／MariaDB 的 8 種 OGC spatial 與 SQL Server geometry／geography 使用共同的 `SRID=<非負整數>;<WKT>` 編輯格式；provider 載入 canonical WKT、顯式重建原生物件，optimistic predicate 同時比對 SRID 與 WKT。MariaDB 對畸形 WKT 只回 warning／NULL 的差異已 fail-closed 包裝為錯誤，nullable 欄位不會被靜默清空。MySQL 8、MariaDB 11.4、SQL Server 2022 實機矩陣涵蓋 NULL、metadata、全部型別 CRUD、SRID 保存、畸形 WKT rollback、衝突與刪除；Linux X11 另從 MariaDB UI 修改 geometry／point 後直接查庫核對。
  - MariaDB 原生 UUID／INET4／INET6：metadata 分別映射為 Guid 與 NetworkAddress 編輯器，載入會明確 cast text，UUID 正規化為小寫 D 格式，INET4 僅接受 IPv4 並以 4-byte binary 保存，INET6 接受 IPv4／IPv6 並將 IPv4 保存為 mapped IPv6。寫入與原值 predicate 使用文字參數及 `CAST(... AS UUID/INET4/INET6)`；錯誤 UUID、把 IPv6／CIDR prefix 寫入 INET4、以及 INET6 prefix／zone identifier 都會在送出前 fail closed。MariaDB 11.4 實機矩陣涵蓋 metadata、底層 HEX、IPv4／IPv6 canonical 化、CRUD、錯誤輸入不落資料與 optimistic conflict；CI 會與 MySQL 8、PostgreSQL 16、SQL Server 2022 同時啟動驗證。Linux X11 另從實際 Table 編輯器修改三種原生型別，畫面重新載入 canonical 值後再直接查庫與底層 HEX 核對。
  - MySQL／MariaDB DATE／DATETIME／TIMESTAMP：metadata 會保留 `DATETIME(p)`／`TIMESTAMP(p)` 的 0–6 位小數秒精度，載入以固定長度 canonical ISO 文字顯示，寫入則使用 Date／DateTime／Timestamp 原生參數。DATE／DATETIME 會先檢查 1000–9999 年範圍；三者都拒絕寬鬆日期格式、offset 與超過欄位 fsp 的小數秒，避免 server 預設在沒有 warning 的情況下四捨五入。MySQL 8 與 MariaDB 11.4 實機矩陣涵蓋四個代表欄位、metadata、1000／9999 年、毫秒／微秒、錯誤輸入不落資料、CRUD 與 optimistic conflict；Linux X11 另驗證 `DATETIME(3)` 第 4 位小數會在 UI 被拒絕，再從 Table 編輯器修改 DATE、兩種 DATETIME 與 TIMESTAMP，重新載入後用原生 client 逐欄查回核對。
  - SQL Server hierarchyid：以最多 1 MiB 的 canonical path 文字編輯，參數固定使用 `nvarchar(max)` 並由 `hierarchyid::Parse()` 權威驗證；載入透過 `ToString()` 顯示 `/1/2.5/` 形式，原值 predicate 使用原生 hierarchyid 等號。SQL Server 2022 實機矩陣涵蓋 NULL、metadata、新增、修改、畸形 path rollback、衝突與刪除；Linux X11 另從 UI 修改 path 後用 `sqlcmd` 直接查庫核對。
  - SQL Server sql_variant：以 `type:value` 編輯並顯式保存內層 base type；支援整數、bit、decimal／numeric precision／scale、money、有限浮點數、date／datetime／smalldatetime／datetime2／datetimeoffset／time、GUID、定長／變長 ANSI／Unicode 字串與 binary。字串 canonical tag 另帶 collation name、LCID 與 comparison style，載入與 optimistic predicate 會比對完整型別 metadata 和內容；寫入依 tag 建立強型別參數，numeric 與 datetimeoffset 再由 SQL Server 明確 CAST，避免 synonym 或時區 offset 被驅動程式改變。時間與金額會在本機先拒絕需要取整的值，binary、字串長度及 decimal 範圍也 fail closed。SQL Server 2022 實機矩陣涵蓋 decimal／numeric、int、nvarchar／varchar custom collation、datetime2／datetimeoffset／time、varbinary 的 metadata、CRUD、越界 rollback、custom-collation 原值鎖定、衝突與刪除，並以 `SQL_VARIANT_PROPERTY` 直接核對修改後的 base type、precision、scale 與 max length。
  - SQL Server 一般字串：`char`／`varchar`／`text`、`nchar`／`nvarchar`／`ntext` 與字串 alias type 的 metadata 會保留 base storage type、最大 bytes 與欄位 collation。所有輸入先用 `nvarchar(max)` 參數無損送達，再由 SQL expression 套用目標 collation；ANSI／UTF-8 型別會驗證 `DATALENGTH` 並轉回 Unicode 做 binary round-trip，Unicode 型別則依 UTF-16 bytes 驗證容量，任何 code-page 替換或截斷都在單列交易內 fail closed。原值 predicate 也以 collation-aware 轉換後的 bytes 比對，補上 SQL Server 一般字串等號忽略尾端空白的競爭衝突缺口。SQL Server 2022 實機矩陣先證明 legacy varchar 原生寫入會把 `漢字` 靜默存成 `??`，再覆蓋 ANSI、UTF-8、Unicode、alias、legacy text 的拒絕、不落資料、合法 byte 邊界 CRUD、尾端空白衝突與刪除。
  - SQL Server scalar money：一般 `money`／`smallmoney` 欄位使用獨立的 fixed-scale 編輯器與原生 `SqlDbType.Money`／`SmallMoney` 參數，不再走無範圍限制的通用 decimal。載入正規化為 4 位小數文字；寫入前依各自正負邊界拒絕溢位，並拒絕會被 SQL Server 四捨五入的第 5 位小數，以及幣別符號、千分位與科學記號。SQL Server 2022 實機矩陣涵蓋兩種型別的 metadata、正負邊界、CRUD、錯誤值不落資料、optimistic conflict 與刪除；Linux X11 另確認 grid 固定 4 位小數、UI 拒絕需取整的第 5 位，合法修改 money／smallmoney 後再由 `sqlcmd` 直接查回一致。
  - SQL Server scalar temporal：`date`、`datetime`、`smalldatetime`、`datetime2(p)`、`datetimeoffset(p)` 與 `time(p)` 使用各自精確的 `SqlDbType`；帶小數秒的型別會依 metadata 設定原生 Scale，不再讓一般 `DateTime` 參數退回舊式 `datetime`。本機解析會拒絕 `datetime` 1753 年以前／無法精確保存的 1/300 秒值、`smalldatetime` 的秒數與範圍，以及超出欄位 Scale 的時間，避免 server 無聲取整。SQL Server 2022 實機矩陣涵蓋全部六類、`datetime2(3)`／`datetime2(7)` 雙精度、metadata、0001 年、100ns、正負 offset、邊界修改、錯誤輸入不落資料、衝突與刪除；Linux X11 另從 UI 先驗證 `datetime2(3)` 多一位小數會被拒絕，再修改七個時間欄位並用 `sqlcmd` 逐欄查回核對。
  - SQL Server alias type：metadata 分開保存 declared type 與 base storage type；`sysname` 會顯示為 `sysname (nvarchar(128))`，`CREATE TYPE dbo.precise_amount FROM decimal(18,6)` 則顯示為 `[dbo].[precise_amount] (decimal(18,6))`。字串、整數與 exact decimal alias 沿用 base type 的參數、precision／scale 與 optimistic predicate，同時保留 alias 名稱；SQL Server 2022 實機矩陣涵蓋 metadata、CRUD、int alias 溢位 rollback、衝突與刪除，Linux X11 另修改 nvarchar／int／decimal alias 與 sysname 後直接查庫核對。
  - 更新檢查與下載：主視窗從最新公開 GitHub Release 依實際 OSArchitecture 選擇四個允許 RID 之一，只有目標安裝包與同名 `.sha256` 都存在才開放下載；release 頁與資產 URL 都必須是 `github.com/shadowjohn/mySQLPunk` 下的 HTTPS，非預期版本、RID、scheme 或 host 會 fail closed。Sidecar 限制 4 KiB、單筆 strict UTF-8 SHA-256 與精確檔名，安裝包限制 512 MiB 並串流寫入／計算雜湊；同目錄暫存通過後才原子替換，任何錯誤或取消都保留既有檔案。Linux 與 macOS UI 都會在關閉前以 `CreateNew` 原子保留 updater lock，多視窗同時套用會 fail closed，已不存在 PID 的 stale lock 才會回收；獨立 updater 取得 token 後等待目前 PID 結束，再以私有副本重驗 SHA-256、archive 路徑、entry 類型、數量與展開大小。Linux 最後執行交易式 installer，macOS 則在目標 bundle 同檔案系統核對 plist 版本、RID、Mach-O 架構與 codesign 後原子換包。新版啟動五秒內退出會回復舊版並重啟，失敗結果與 log 位置會在 UI 顯示；Developer ID/notarization 仍為需要發行憑證的獨立工作。
  - 驗證與發佈：Core smoke tests 覆蓋密碼不落地、三種結果格式、Unicode、escaping、公式注入、JSON 型別、重複欄名、失敗不覆寫、兩種系統工具的 store/get/delete 與 arguments 安全契約、SQLite DDL / DML / UTF-8 SELECT、metadata、Table Value／NULL／DEFAULT／binary／JSON／XML CRUD、無 PK 限制、參數格式、大小限制、畸形 JSON、XML 多根節點、DTD／XXE 與深度限制，以及 MySQL BIT width／UInt64 邊界、ENUM／SET 原生參數／宣告順序／strict 拒絕、TIME 正負／超過 24 小時／宣告微秒精度／±838:59:59 邊界，PostgreSQL bit string 固定／可變長度、timetz 微秒／正負 offset、interval 三分量／正負值／欄位限制／小數秒精度、pg_lsn 32-bit 分量／64-bit 最大值、oid／xid／cid UInt32 與 xid8 UInt64 邊界、tsvector 位置／權重、tsquery prefix／NOT／phrase-distance、server 拒絕畸形全文查詢且不落資料、IPv4／IPv6／CIDR／MAC 正規化與錯誤輸入、SQL Server hierarchyid canonical path／大小／NUL／server rollback、sql_variant tag／型別 metadata／日期金額精度／custom collation 原值比對，以及 scalar temporal 範圍／Scale／無損表示、競爭衝突、跨三頁 offset／邊界狀態、更新版本比較、四種 RID 命名、安裝包／SHA-256 配對、不受信任 URL 拒絕、Linux／macOS updater ArgumentList 與失敗結果解析。Linux X11 已從 UI 完成 SQLite Table 的新增、修改 NULL、binary hex、JSON、XML、MariaDB ENUM／SET／完整範圍 TIME／UUID／INET4／INET6、SQL Server hierarchyid／scalar temporal，以及 PostgreSQL bit string、interval、pg_lsn、四種系統識別碼、tsvector／tsquery 與四種網路位址修改後直接查庫確認、確認刪除、200 列翻到 201–400 列並驗證畫面；Docker 實機矩陣會在 Linux x64 啟動 MySQL 8、MariaDB 11.4、PostgreSQL 16 與 SQL Server 2022，驗證建庫、DDL、DML、Unicode、metadata、實際分頁 SQL及安全 CRUD／衝突情境後自行清理。GitHub Actions 會在原生 Linux x64／ARM64 與 Intel／Apple Silicon macOS runner 建立 self-contained `linux-x64`／`linux-arm64` tar 與 `osx-x64`／`osx-arm64` `.app.zip`，每個資產附獨立 SHA-256。兩個 Linux job 都會執行交易式安裝故障注入、隔離安裝、Xvfb UI 啟動、解除安裝、安全更新、五秒啟動檢查、rollback、舊版重啟與失敗狀態檔；兩個 macOS job 分別從 ZIP 重建同架構 app bundle，核對 plist、CPU 架構與 codesign，再以健康新版、立即退出新版、錯誤 SHA 與 symlink 檔案驗證安全套用／rollback，最後實際套用 self-contained app 並完成啟動健康檢查。tag Release 會先完成全部驗證與四種跨平台資產，再由 Windows job 核對九個檔案齊全並一次發布。macOS Developer ID/notarization 與 Windows 完整版其餘 provider／工作台能力留待後續。
  - Docker 實機覆蓋明細：含 binary、MySQL／MariaDB JSON／BIT(8)／BIT(64)／ENUM／SET／TIME(6)、MariaDB UUID／INET4／INET6、PostgreSQL jsonb／XML／bit／bit varying／timetz／interval／受限 interval typemod／pg_lsn／oid／xid／cid／xid8／tsvector／tsquery／inet／cidr／macaddr／macaddr8，以及 SQL Server XML／text／ntext／image／hierarchyid／sql_variant／scalar temporal 的安全 CRUD 與衝突情境。
...
- **Redis 有序集合刪除衝突保護**
  - 刪除會在 WATCH 後比對畫面載入時的分數，分數改變、成員消失或 EXEC 被中止都回報衝突；分數以數值比較，`7` 與 `7.0` 視為相同。
  - 回歸案例涵蓋過期分數、等值文字、成員消失及交易中止；Redis／Garnet 實機矩陣另加入外部改分數後拒絕刪除的案例。

- **Redis list 元素安全刪除**
  - 刪除前以 WATCH 鎖定 key 並比對畫面載入時的索引和值；交易中先把該索引換成兩組 GUID 組成的唯一標記，再只移除一次標記，因此 list 內有重複值也不會刪到別的位置。
  - 索引消失、值被外部修改或 EXEC 中止都會回報衝突且保留清單；loopback 測試涵蓋重複值、過期值、越界索引與交易中止，Redis／Garnet 實機矩陣也會驗證重複值及外部修改。

- **Redis／Garnet INFO 即時監控**
  - Redis 資料庫節點可開啟停靠式監控頁，支援手動更新或每 1／5／10／30 秒自動更新；摘要列出 logical db key 數、版本、運行時間、連線與阻塞數、記憶體、每秒命令、命中率、網路流量、CPU、持久化與複寫狀態。
...
- **Navicat 對齊：兩庫結構差異報告 ✅ 第一版已完成**
  ...
  - 視覺查詢建構器：資料庫右鍵「查詢建構器...」以畫布加入資料表（外鍵自動連接、拖曳欄位建立連接），設定輸出、別名、彙總、排序、分組、條件、DISTINCT 與筆數上限；SQL 依 provider 即時產生，可修改後轉回圖形（不支援的語法明確拒絕），可複製或送到查詢視窗，本身不執行語句。
  - MongoDB 結構描述分析：collection 右鍵「結構描述分析...」抽樣（前 N 筆或 `$sample`）後展開巢狀與陣列路徑，列出出現率、型別分佈、NULL、範圍、常見值與 IQR 極端值（附 `_id`），並標出混合型別、數字存成字串、稀疏欄位、大小寫不一致欄位名稱、空字串等異常；只讀取文件。
  - MongoDB Aggregation Pipeline：collection 右鍵「Aggregation Pipeline...」以範本新增唯讀 stage、調整順序與啟用、逐 stage 編輯 JSON，預覽到任一 stage 的前 N 筆輸出；拒絕 $out／$merge（含巢狀），可匯入、複製 mongosh 語法或送到查詢視窗（查詢 JSON 支援 pipeline 欄位）。
...
- **查詢編輯器詢問 AI 與錯誤交接 ✅ 已完成**
  ...
  - SQL 動作：同一選單新增格式化與資料庫語法轉換；格式化提示明確限制只能調整排版、縮排與關鍵字大小寫，轉換則可選 MySQL／MariaDB、PostgreSQL、SQL Server、Oracle 或 SQLite，並把來源與目標 provider 一起放進草稿。目前使用中的 provider 會停用，避免同方言轉換。
  - 自訂動作：「管理自訂動作」可新增、修改、刪除提示，並決定是否釘選到「詢問 AI」選單；未釘選的動作仍保留在管理視窗，需要時可直接按「儲存並使用」。執行時會沿用目前選取範圍或全文，AI 回覆若含 SQL 也會進入同一套差異確認流程。
  - 保存範圍：自訂動作寫入使用者目錄的 `query-ai-actions.json`，只包含名稱、提示內容與釘選狀態，不保存編輯器 SQL、查詢結果、連線資訊或 AI 認證；同名動作會拒絕，並限制最多 50 組、名稱 80 字、提示 4,000 字。
  ...
  - 差異確認：最佳化與修正回覆若含 SQL，可先逐行並排比較原始內容與 AI 建議；連續差異會合併成一個變更區段並預設全選，可取消不想採用的區段，再按「套用已選變更」回寫當次選取範圍或全文，且不會自動執行。
  - 部分套用：取消修改區段會保留原行，取消新增區段會略過新行，取消刪除區段則保留原本要刪除的行；重建 SQL 時沿用原編輯器的換行格式，沒有選取任何區段時不開放套用。
  - 版本保護：送出 AI 前會記住完整編輯器快照，開啟差異與實際寫入前各檢查一次；等待回覆期間或預覽期間只要 SQL 被修改，就拒絕用舊建議覆蓋。
  - 驗證：smoke test 覆蓋格式化限制、來源／目標 provider、缺少轉換目標時 fail closed、五種目標選單與同 provider 停用；也涵蓋自訂動作 JSON round-trip、更新／刪除、重名拒絕、釘選篩選、損毀檔案 fail closed、提示長度上限、繁中管理視窗，以及查詢工具列只顯示釘選項目。
...
- **Punky 多聊天室 ✅ 第一階段已完成**
  - 操作：AI 面板可新增、切換、重新命名與關閉對話；第一則送出的問題會成為簡短標題，之後仍能自行修改。最後一段對話不允許關閉，有內容或草稿的對話會先確認再移除。
  - 隔離：每段對話分開保存訊息上下文、未送出草稿、schema 勾選、最後一段 AI SQL 與其差異套用回呼；切換後不會把前一段的 SQL 套用動作帶過去。模型回覆期間暫停聊天室與供應商切換，回覆仍寫回原本的對話。
  - 隱私：聊天室只存在本次程式的記憶體，不寫入設定檔或其他磁碟檔案；關閉程式後不保留。
  - 驗證：smoke test 覆蓋新增、切換、草稿與 schema 選擇還原、SQL 回呼隔離、重新命名、單一對話保護與關閉後狀態還原。

- **跨模型答案並排比較 ✅ 已完成**
  - 入口：Punky 輸入區的「比較」會開啟獨立寬視窗，左右兩側可分別選 provider 與 model；可比較同一服務的不同模型，也能跨 Codex／Claude／Gemini CLI、API 或本機 OpenAI 相容服務。
  - 請求內容：兩側各自收到相同的 Punky system prompt、勾選時取得的同一份 schema 快照、目前聊天室最近 12 則 user／assistant 訊息，以及輸入框尚未送出的問題；聊天室內既有 system 訊息不會被當成模型指令轉送。
  - 結果：兩個呼叫並行執行，各自計一次用量並在左右欄顯示；單側失敗只顯示在該欄，答案可用右鍵複製。結果不寫入聊天室歷史、不清除草稿、不自動執行或套用 SQL。
  - 設定與安全：送出前會拒絕相同的 provider／endpoint／model、未知 provider、缺少 endpoint／model 或 API key；只保存右側 provider ID 與模型名稱供下次選用，不保存提示、schema、答案或認證。比較進行中不把關閉視窗誤當成取消，必須等兩側完成。
  - 驗證：smoke test 覆蓋設定正規化、相同模型與缺少端點 fail closed、上下文複製與 system 訊息過濾、左右分割結果區、預設挑選不同模型，以及 380px AI 面板仍保留可用輸入寬度。
...
- **Navicat 對齊：資料分析 ✅ 欄位統計、Top 值分佈與互動鑽取已完成**
  - 現況：資料表節點與資料表清單右鍵新增「資料分析」，以可停靠工作區顯示五種既有 provider 的欄位型別、分析筆數、NULL、相異值、最小值、最大值、數值平均與 Top 10 值比例。
  - 安全與效能：預設只分析 10,000 筆，可切換 1,000／50,000／全表；provider 不支援 BLOB／JSON 等型別的 DISTINCT、排序或分組時，保留可取得的統計並標示部分完成，不讓整份報告失敗。
  - 互動：選取欄位會切換 Top 值分佈，選取或雙擊值可開啟帶安全字面值與 200 筆上限的 WHERE 查詢；MySQL、PostgreSQL、SQLite、SQL Server、Oracle 會使用各自的 identifier quoting 與 LIMIT／TOP／ROWNUM／FETCH 語法。
  - 後續方向：加入字串格式異常、長度分佈、日期／數值區間圖與自訂抽樣方式。
...
- **模型總覽表 ✅ 已語系化**
  ...
  - 完成內容：物件類型、就緒狀態與 Models 清單描述已改用語系字串，繁中會顯示「資料表 / 檢視 / 就緒」，英文維持 `Table / View / Ready`。

- **模型欄位與索引目錄 ✅ 已語系化**
  - 現況：`模型 > Column Catalog` 會列出資料表/檢視欄位，`模型 > Index Catalog` 會列出索引或無明確索引狀態。
  - 完成內容：欄位目錄的物件類型、索引目錄的無索引 fallback 與 Models 清單描述已改用語系字串，繁中與英文介面都會顯示一致文字。
...
- **資料庫群組清單 ✅ 已修正並語系化**
  ...
  - 完成內容：群組清單會正確指定到右側資料表資料來源；檢視、模型、其它、查詢、報表、SQL 備份、缺少來源、空白、就緒狀態與描述欄位已改用語系字串。
...
- **物件細節面板標題 ✅ 已語系化**
  - 現況：右側資訊面板標題會顯示目前選取的資料庫、群組、資料表、檢視、事件、函式、使用者、模型、BI、其它工具、報表或搜尋範圍。
  - 完成內容：標題類型與分隔符已改用共用語系 helper，繁中會顯示全形冒號，英文會維持半形冒號，並已納入 smoke test 覆蓋。

- **物件細節面板屬性列 ✅ 已語系化**
  - 現況：右側資訊面板的屬性表會顯示資料庫資訊、資料表狀態、檢視、事件、函式、使用者、模型、BI、其它工具與報表的欄位名稱。
  - 完成內容：屬性 / 值欄位標題，以及字元集、列數、引擎、回傳型別、主機、來源、資料庫、Provider 等屬性名稱已改用語系字串，繁中與英文介面都會顯示一致文字，並已納入 smoke test 覆蓋。

- **物件細節面板語系切換刷新 ✅ 已補齊**
  - 現況：右側資訊面板的資訊 / DDL 切換按鈕與屬性表欄位標題會隨目前語系顯示。
  - 完成內容：細節面板工具列按鈕文字與屬性 / 值欄位標題已接到 `ApplyLanguage()`，切換繁中 / 英文後會立即刷新，並已納入 smoke test 覆蓋。

- **物件細節面板錯誤訊息 ✅ 已語系化**
  - 現況：右側資訊面板載入資料表、檢視、事件、函式與使用者細節時，可能顯示載入失敗或找不到物件訊息。
  - 完成內容：載入失敗與事件 / 函式 / 使用者找不到訊息已改用語系字串，繁中與英文介面都會回饋一致文字。

- **物件細節面板 DDL 註解標頭 ✅ 已語系化**
  - 現況：右側 DDL / 文字面板會替使用者、模型、BI、其它工具與報表顯示註解式標頭與描述。
  - 完成內容：User / Model / BI / Other / Report 標頭、來源與狀態欄位標籤已改用物件類型與屬性語系字串，繁中與英文介面都會顯示一致文字，並已納入 smoke test 覆蓋。
...
- **主視窗狀態列操作回饋 ✅ 已語系化**
  - 現況：SQLite + SpatiaLite 連線成功、查詢歷程載入、SpatiaLite runtime 修復腳本啟動、查詢分頁開啟與資料庫群組缺失等流程會在主視窗狀態列顯示操作回饋。
  - 完成內容：上述狀態列訊息已集中改用語系 helper，繁中與英文介面都會依目前語系顯示一致文字，並已納入 smoke test 覆蓋。

- **查詢分頁右鍵選單 ✅ 已語系化**
  - 現況：查詢分頁可透過右鍵選單浮動 / 取消停靠或關閉。
  - 完成內容：浮動 / 取消停靠選單文字已改用語系字串，繁中與英文介面都會依目前語系顯示一致文字，並已納入 smoke test 覆蓋。

- **資料表資料底部工具列提示 ✅ 已語系化**
  - 現況：資料表資料模式底部工具列以符號按鈕提供新增、刪除、儲存、取消、重新整理與分頁操作。
  - 完成內容：底部工具列按鈕已補上繁中 / 英文 tooltip，切換語系或關閉「顯示工具提示」後會立即刷新，並已納入 smoke test 覆蓋。

- **全域錯誤與 BLOB 串流進度回饋 ✅ 已語系化**
  - 現況：UI 執行緒、背景執行緒未處理例外會顯示全域錯誤對話框；資料表模式匯出大型 BLOB 時會在查詢視窗狀態列顯示串流進度。
  - 完成內容：全域錯誤標題、UI/背景錯誤訊息與 BLOB 串流匯出進度已改用語系字串，繁中與英文介面都會依目前語系顯示一致文字，並已納入 smoke test 覆蓋。

- **關於視窗內容 ✅ 已語系化**
  - 現況：`說明 > 關於` 會以自訂視窗顯示版本、平台、支援連線、作者資訊與看板娘 Punky 崩琦眨眼動畫。
  - 完成內容：產品定位、版本、平台、支援連線、作者標題、看板娘標題與副標已改用語系字串，繁中與英文介面都會依目前語系顯示一致文字；動畫以約 16 fps 透明 GIF 打包進 `image/` 並已納入 smoke test 覆蓋。
...
- **連線編輯視窗 ✅ 已語系化**
  - 現況：MySQL、PostgreSQL、Oracle、SQLite 與 SQL Server 連線編輯 / 新增視窗會顯示分頁、欄位標籤、測試連線、確定與取消等控制項。
  - 完成內容：上述五個 provider 的連線表單已補齊繁中 / 英文控制項文字，Oracle 的 Service Name、Net Service Name 等欄位也已改為可語系化顯示；測試連線與 SQLite 初始化失敗會保留明確錯誤原因，若 provider 例外沒有明確訊息則依目前語系回退為「未知錯誤 / Unknown error」，並已納入 smoke test 覆蓋實際表單控制項與錯誤訊息。
...
- **連線匯出/匯入與 SQL 匯入錯誤 ✅ 已語系化**
  - 現況：匯入連線設定前會先建立差異預覽，來源檔不存在時會回報錯誤；連線匯出/匯入與 SQL 匯入若底層例外沒有明確訊息，也需要避免前端只顯示空白原因。
  - 完成內容：連線匯入預覽缺少來源檔的錯誤已改用語系字串；連線匯出、連線匯入與 SQL 匯入的錯誤對話框/狀態列會保留明確例外訊息，缺少或空白時依目前語系回退為「未知錯誤 / Unknown error」，繁中與英文介面都會顯示一致文字。
...
- **更新、註冊、連線能力與串流匯出錯誤 ✅ 已語系化**
  - 現況：應用程式更新檢查、進階註冊、連線能力測試與串流匯出會在缺少 GitHub/release/下載路徑參數、應用程式路徑、proxy 測試失敗或遇到不支援格式/provider 時回報錯誤。
  - 完成內容：GitHub owner / repository、Release JSON、下載資料夾、檔案路徑、應用程式路徑、進階註冊套用失敗、proxy 連線測試失敗、串流匯出格式與 provider 不支援等錯誤已改用語系字串；若進階註冊或 proxy 測試例外沒有明確原因，會依目前語系回退為「未知錯誤 / Unknown error」，繁中與英文介面都會顯示一致文字。
...
- **SQLite 欄位註解審核報表資料夾錯誤 ✅ 已語系化**
  ...
  - 完成內容：審核摘要與審核報表輸出資料夾缺失錯誤已改用語系字串，繁中與英文介面都會顯示一致文字。
...
- **由主機節點新增/刪除資料庫只支援部分 provider ✅ 高風險刪除備份、壓縮封存、還原入口與保留上限已補齊**
  ...
  - 完成內容：還原摘要已包含物件數、資料列數、欄位 schema 差異，且摘要標題、差異類型、欄位變更、內容指紋、比對/抽樣覆蓋列數與缺值文字都會依繁中/英文語系切換；資料表會在還原前後建立順序無關的 SHA-256 內容指紋，即使列數不變但資料值被改動，也會在「資料內容差異」中標示變更的資料表。內容指紋改用 provider 分頁讀取，每頁先計算 row hash 再組成摘要，避免一次把大型資料表整批載入記憶體；`選項 > 檔案位置` 可設定內容指紋最多抽樣列數，預設 10000 列；還原完成後也會輸出 JSON 內容掃描報表到文件目錄的 `mySQLPunk/restore-content-scan-reports`，保留每張資料表的列數、抽樣列數、是否完整比對與前後 SHA-256 指紋，方便事後稽核。
...
- **沒有 Primary Key 的資料表儲存風險較高 ✅ 唯讀選項、optimistic WHERE 與衝突值提示已補齊**
  ...
  - 完成內容：`選項 > 一般` 新增「沒有 Primary Key 的資料表以唯讀模式開啟」。啟用後，開啟無 Primary Key 的資料表會自動停用新增、刪除與儲存操作，並在狀態列提示原因；若使用者選擇繼續編輯，衝突訊息會補充 UPDATE/DELETE、比對條件摘要與參數值（包含原始比對值與本次異動值），避免只看到泛用錯誤。若 UPDATE/DELETE 影響 0 列，QueryForm 會用未被本機修改且可安全比對的原始欄位重查候選資料列；剛好找到一列時會顯示原始值與目前資料庫值的欄位差異，找到 0 列或多列時會明確提示無法安全判斷。查詢執行、分頁載入、儲存資料、匯出與資料列寫入若收到空白例外或空白 provider reason，也會依目前語系回退成「未知錯誤 / Unknown error」，避免結果表格、狀態列或錯誤視窗只顯示空白原因。
  - 風險：資料列被其他人改過，或欄位包含浮點/大文字時，WHERE 仍可能因 provider 比對規則而不穩定。
  - 後續方向：若仍需要編輯無 Primary Key 資料表，可再補更多 provider 實機案例與衝突情境測試。

- **BLOB/geometry 欄位操作 ✅ 分頁檢視、資料表模式串流匯出、查詢結果底層串流匯出與預覽語系化已補齊**
  - 現況：`byte[]` 欄位在結果表格中會先嘗試顯示為 Geometry WKT 預覽，無法解析時才顯示 BLOB 大小與十六進位片段；右鍵可檢視十六進位、複製 Hex、匯出檔案，在資料表資料模式可匯入檔案寫回目前 BLOB 欄位，也可針對 geometry 複製 WKT / WKT 轉 Geometry SQL。
  ...
  - 完成內容：`QueryResultExportService` 新增 provider-level streaming export 底層能力，可直接用 `DbDataReader` 將任意查詢結果輸出為 CSV / TSV / JSON / XML / HTML / Markdown，不必先載入整份 `DataTable`，並沿用既有 BLOB/geometry 預覽格式、空值處理與列數進度回報；查詢視窗匯出上述文字格式時，若上一個成功結果是一般查詢且可安全重跑，會直接走串流匯出，資料表編輯模式與 XLSX 則保留既有 `DataTable` 路徑；主視窗資料格、查詢視窗資料格與匯出 service 的 BLOB/geometry fallback 預覽已改用語系字串，繁中會顯示「幾何」與「位元組」，英文維持技術標籤；匯出完成視窗的「開啟檔案 / 開啟資料夾」失敗時也會依目前語系顯示可讀錯誤，缺少例外原因時會回退為「未知錯誤 / Unknown error」。
...
- **查詢結果匯出格式 ✅ 已補齊常用格式**
  ...
  - 完成內容：各格式會共用結果表格顯示值轉換；BLOB/geometry 會沿用結果表格的語系化 Geometry WKT 或 BLOB 十六進位片段顯示，日期與空值也會一致處理；XLSX 匯出會凍結標題列、套用自動篩選、穩定欄寬與表頭樣式，讓大型 workbook 開啟後可直接篩選與辨識欄位；SQL INSERT 匯出會產生 `query_result` 目標表的 `INSERT INTO` 腳本，字串、NULL、數字與 BLOB 十六進位 literal 會正確輸出，且可走串流匯出大型結果；匯出完成後會顯示摘要視窗，列出格式、筆數、檔案大小與路徑，摘要 helper 與對話框欄位都會依目前語系顯示，並可直接開啟檔案或所在資料夾；成功匯出後會記住這次選擇的資料夾，下次匯出會直接從同一個位置開始。
...
- **View SQL 無法安全轉換時會改用 table snapshot ✅ 使用者選項與預覽已補齊**
  ...
  - 完成內容：複製前新增「跨 Provider 複製 View」對話框，讓使用者選擇：
    ...
    - 取消複製
    - 可在同一個對話框檢查來源 View SQL 與轉換後 SQL 預覽；若無法安全轉換，會顯示原因；若取得來源 View SQL 時 provider 例外原因空白，會依目前語系回退為「未知錯誤 / Unknown error」。
