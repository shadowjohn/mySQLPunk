...
- **Linux / macOS 跨平台預覽 🟡 第二階段進行中**
  ...
  - UI：連線設定新增／編輯／刪除、SQLite 檔案選擇、資料庫切換、Table / View 物件樹、Ctrl+Enter／按鈕執行、取消操作、動態結果網格與狀態／錯誤顯示。雙擊 Table 會開啟獨立的 200 列資料編輯器，雙擊 View 則產生預覽 SQL；編輯器依 metadata 顯示 PK、generated、nullable 與型別，支援 Value／NULL／DEFAULT 新增、修改、刪除與重新整理。
  ...
  - 更新檢查：主視窗提供手動更新檢查，從最新公開 GitHub Release 依實際 OSArchitecture 選擇四個允許 RID 之一，只有目標安裝包與同名 `.sha256` 都存在才顯示完整配對；release 頁與資產 URL 都必須是 `github.com/shadowjohn/mySQLPunk` 下的 HTTPS，非預期版本、RID、scheme 或 host 會 fail closed。現階段只開啟下載頁，不在程式執行中自動覆寫 app，待補安全套用與 rollback。
  - 驗證與發佈：Core smoke tests 覆蓋密碼不落地、三種結果格式、Unicode、escaping、公式注入、JSON 型別、重複欄名、失敗不覆寫、兩種系統工具的 store/get/delete 與 arguments 安全契約、SQLite DDL / DML / UTF-8 SELECT、metadata、Table Value／NULL／DEFAULT CRUD、無 PK 限制、參數格式驗證與競爭衝突，以及更新版本比較、四種 RID 命名、安裝包／SHA-256 配對與不受信任 URL 拒絕。Linux X11 已從 UI 完成 SQLite Table 的新增、修改 NULL、確認刪除並直接驗證資料庫內容；Docker 實機矩陣會在 Linux 啟動 MySQL 8、PostgreSQL 16 與 SQL Server 2022，驗證建庫、DDL、DML、Unicode、metadata 與同一套安全 CRUD／衝突情境後自行清理。GitHub Actions 會在 Ubuntu 與 macOS 原生建立 self-contained `linux-x64`／`linux-arm64` tar 與 `osx-x64`／`osx-arm64` `.app.zip`，每個資產附獨立 SHA-256；Linux x64 執行隔離安裝、Xvfb 啟動與解除安裝，ARM64 執行不可啟動的檔案層安裝／解除安裝，macOS 則從 ZIP 重建 app bundle 後核對 plist、CPU 架構與 codesign。tag Release 會先完成四種跨平台資產，再由 Windows job 核對九個檔案齊全並一次發布。macOS Developer ID/notarization、更新安全套用／rollback、Linux ARM64 實機啟動與 Windows 完整版其餘 provider／工作台能力留待後續。
