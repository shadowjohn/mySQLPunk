...
- **Linux / macOS 跨平台預覽 🟡 第二階段進行中**
  ...
  - 安全資料編輯：新增、修改與刪除只接受 schema metadata 中存在且可編輯的欄位，所有值使用 provider 參數，不把輸入拼入 SQL。修改／刪除要求 Primary Key，WHERE 同時比對 PK 與載入時的可編輯原始值；影響 0 列視為外部變更衝突，超過 1 列也會回復交易，避免靜默覆蓋或批次誤改。MySQL／MariaDB BLOB、PostgreSQL bytea、SQL Server binary／varbinary 與 SQLite BLOB 接受 `0x` 加偶數個 hex 字元並以 `DbType.Binary` 寫入，上限 1 MiB；MySQL／MariaDB JSON、PostgreSQL json／jsonb 與 SQLite JSON 會先經嚴格 JSON parser 驗證，再以 provider 專用參數寫入，並針對 MySQL 儲存正規化與 PostgreSQL json 無原生等號的差異使用正確原值 predicate。PostgreSQL xml、SQL Server xml 與 SQLite XML 會禁止 DTD／外部實體並限制文件深度，原生 provider 另使用 XML 參數與可比較的序列化原值 predicate。PostgreSQL inet／cidr／macaddr／macaddr8 會驗證 address family、prefix、CIDR host bits、zone identifier 與 MAC byte 長度，再轉成 Npgsql 原生型別；載入時由 PostgreSQL 轉成文字以保留 inet prefix。超過 1 MiB 的既有 binary／JSON／XML 只顯示摘要，且不放入非 PK optimistic predicate。無 Primary Key 的 Table 只允許新增與瀏覽，generated 及尚未支援的進階型別維持唯讀。
