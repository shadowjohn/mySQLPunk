...
- **Linux / macOS 跨平台預覽 🟡 第二階段進行中**
  ...
  - PostgreSQL 文字序列化型別：`jsonpath`、`pg_snapshot`／`txid_snapshot`、`hstore`、`ltree`／`lquery`／`ltxtquery` 與 11 種 `reg*` 物件參照改以最多 1 MiB 的純文字編輯；參數保持 Unknown，讓欄位型別上下文交由 PostgreSQL 權威 parser 驗證。載入一律轉 canonical text；`jsonpath` 與 snapshot 等沒有原生等號的型別，以載入時文字做 optimistic predicate，仍可偵測其他連線改動。PostgreSQL 16 實機矩陣涵蓋全部 18 個型別的 metadata、新增、修改、畸形 jsonpath rollback、衝突與刪除，Linux X11 另從 UI 修改 jsonpath／ltree 後直接查庫核對。
