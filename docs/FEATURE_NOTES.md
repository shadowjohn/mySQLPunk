...
- **Linux / macOS 跨平台預覽 🟡 第二階段進行中**
  ...
  - 結果匯出：結果區可明確選擇 CSV、TSV 或 JSON，再由 Linux / macOS 原生儲存對話框選擇檔案。CSV / TSV 採 UTF-8 BOM 與 CRLF，正確處理分隔符、引號、換行、空字串、NULL、日期、數字與 binary hex，並中和 `= + - @` 等試算表公式注入；JSON 不帶 BOM，保留 JSON 可表達的型別並為重複欄名建立穩定唯一名稱。輸出先寫同目錄暫存檔，成功後才替換目標；截斷結果會在匯出前提示只包含目前載入的前 10,000 列。
