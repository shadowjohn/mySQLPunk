...
- **查詢編輯器詢問 AI 與錯誤交接 ✅ 已完成**
  ...
  - 差異確認：最佳化與修正回覆若含 SQL，可先逐行並排比較原始內容與 AI 建議；使用者按下「確認套用」後才會回寫當次選取範圍或全文，且不會自動執行。
  - 版本保護：送出 AI 前會記住完整編輯器快照，開啟差異與實際寫入前各檢查一次；等待回覆期間或預覽期間只要 SQL 被修改，就拒絕用舊建議覆蓋。逐段勾選仍留在後續階段。
