...
- **Linux / macOS 跨平台預覽 🟡 第二階段進行中**
  ...
  - 驗證與發佈：Core smoke tests 覆蓋密碼不落地、三種結果格式、Unicode、escaping、公式注入、JSON 型別、重複欄名、失敗不覆寫、兩種系統工具的 store/get/delete 與 arguments 安全契約、SQLite DDL / DML / UTF-8 SELECT、metadata、Table Value／NULL／DEFAULT／binary／JSON CRUD、無 PK 限制、參數格式、binary／JSON 大小與畸形 JSON 驗證、競爭衝突、跨三頁 offset／邊界狀態，以及更新版本比較、四種 RID 命名、安裝包／SHA-256 配對、不受信任 URL 拒絕、Linux updater ArgumentList 與失敗結果解析。Linux X11 已從 UI 完成 SQLite Table 的新增、修改 NULL、binary hex 與 JSON 修改後直接查庫確認、確認刪除、200 列翻到 201–400 列並驗證畫面；Docker 實機矩陣會在 Linux 啟動 MySQL 8、PostgreSQL 16 與 SQL Server 2022，驗證建庫、DDL、DML、Unicode、metadata、實際分頁 SQL，以及含 binary、MySQL JSON 與 PostgreSQL jsonb 的安全 CRUD／衝突情境後自行清理。GitHub Actions 會在 Ubuntu 與 macOS 原生建立 self-contained `linux-x64`／`linux-arm64` tar 與 `osx-x64`／`osx-arm64` `.app.zip`，每個資產附獨立 SHA-256；Linux 安裝會把版本 app、launcher 與 desktop entry 作為單一交易，並以故障注入驗證任一切換失敗時三者都回復且不留 staging；Linux updater 另以健康新版與立即退出新版驗證套用、五秒啟動檢查、rollback、舊版重啟與失敗狀態檔。Linux x64 另執行隔離安裝、Xvfb 啟動與解除安裝，ARM64 執行不可啟動的檔案層安裝／解除安裝，macOS 則從 ZIP 重建 app bundle 後核對 plist、CPU 架構與 codesign。tag Release 會先完成全部驗證與四種跨平台資產，再由 Windows job 核對九個檔案齊全並一次發布。macOS Developer ID/notarization／自動套用、Linux ARM64 實機啟動與 Windows 完整版其餘 provider／工作台能力留待後續。
