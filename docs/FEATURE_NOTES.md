...
- **Linux / macOS 跨平台預覽 🟡 第二階段進行中**
  ...
  - SQLite temporal：`DATE`、`TIME`、`DATETIME` 與 `TIMESTAMP` 不再先轉成 .NET `DateTime`；編輯器分別要求純 `yyyy-MM-dd`、`HH:mm:ss[.fffffff]`，或不含 offset 的 `yyyy-MM-dd[ T]HH:mm:ss[.fffffff]`，再以 TEXT 參數原樣保存。這避免 Date 被補午夜、Time 被注入當天日期、offset 被轉成本機時間後丟失，以及超過 7 位小數秒。舊有非 canonical／INTEGER／REAL 值只要未修改就以原始顯示判定相同，不會在修改其他欄位時被連帶重寫。SQLite 端到端測試涵蓋四種 metadata、TEXT storage class、7 位小數秒、錯誤輸入不落資料與 optimistic conflict；Linux X11 另先重現 `TIME` 注入日期與 `DATE` 補午夜，再確認修正版拒絕日期時間型 TIME，合法純日期／時間／日期時間保存後以原生 client 核對文字與 hex。
