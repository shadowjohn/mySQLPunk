...
| 模型與資料庫雙向比較／同步 | 🟡 | 兩個已連線資料庫的唯讀結構差異報告與同步 DDL 預覽已完成；待補 DB→模型、模型→DB 與逐項審核執行。 |
...
| 自動執行：查詢、匯入／匯出、傳輸、通知郵件 | 🟡 | 已有查詢、匯出、備份、CSV 匯入與跨庫傳輸（檢查點續傳）作業，失敗重試（匯入寫入部分資料後不重試）、Webhook 與郵件通知（SMTP 密碼存 Windows 認證管理員）、立即執行、每日 Windows 工作排程與 JSON 紀錄；待補每日以外的觸發條件。 |
