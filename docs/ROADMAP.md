...
| 詢問 AI：可自訂／釘選動作 | 🟡 | 已能提問、插入回覆 SQL，查詢工具列也有解釋／最佳化／修正錯誤快捷入口；待補自訂動作管理與釘選。 |
| 解釋／最佳化／格式化／跨資料庫轉換 SQL，差異並排確認 | 🟡 | 已有格式化、AI 對話、查詢工具列解釋／最佳化草稿及 View SQL 跨 provider 轉換；待補統一差異檢視、逐段選取與確認後套用。 |
| AI 修正 SQL 錯誤 | 🟡 | 查詢失敗後可一鍵把當次 SQL、provider、資料庫與已遮蔽敏感值的錯誤帶進 AI 草稿；待補回覆差異並排與確認後回寫。 |
