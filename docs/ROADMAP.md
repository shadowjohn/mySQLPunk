...
> 最新安全進度：MySQL 8、MariaDB 11.4 與 PostgreSQL 16 的一般字串已使用 byte-exact 原值比對，可在大小寫／重音不敏感 collation 下支撐 optimistic concurrency 衝突；PostgreSQL `citext` 也可安全載入與編輯。Linux X11 已實際操作驗證，macOS 由原生 Intel／Apple Silicon CI 持續建置與檢查 app archive。
