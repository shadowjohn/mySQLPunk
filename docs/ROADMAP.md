...
| 同一工作區多模型、Function／Procedure 物件 | ✅ | Windows ER 模型工作區可在同一個 .punkmodel 保存多張圖表；模型結構另含函式與預存程序（從資料庫擷取完整定義，可在模型中新增／修改／刪除），同步到 MySQL、PostgreSQL（含多載）與 SQL Server 時產生建立／更新／刪除語句並逐句審核。 |
| 圖表樣式、圖層、鎖定、群組、自動排列、連接線重導 | ✅ | Windows 版：拖曳移動、依外鍵分層的自動排列、群組（顏色、顯示／隱藏、鎖定）、SVG 向量匯出，以及拖曳連接線的垂直段落手動重導（隨圖表保存、SVG 同步、右鍵可重設，自動排列時清除）。 |
...
| 關聯式／維度／Data Vault 2.0 模型 | ✅ | Windows ER 模型工作區：資料表可標記事實／維度或 Hub／Link／Satellite 角色（表頭顏色與標籤，SVG 同步），可依名稱與外鍵結構推測角色；「產生 Data Vault 2.0」把選取的關聯式資料表轉成 Hub、Satellite（依載入時間保留歷史）與每個外鍵一個 Link，放進新圖表，再以模型同步建立到資料庫。 |
...
| 資料分析：型別、格式、分佈、統計與互動探索 | 🆕 | 資料表右鍵「資料分析」；五種既有 provider 共用，含抽樣／全表、NULL、相異值、極值、平均、Top 10 比例與值鑽取查詢；文字欄位另做格式異常偵測（Email、URL、電話、日期、UUID、IPv4、數字文字與字元樣式，列出不符主要格式的值、前後空白、占位值與只差大小寫的值）。待補更多圖表。 |
...
| 資料產生器（規則、約束、參照完整性、大量資料） | 🟡 | Windows 與 Linux／macOS 皆已完成多表規則編輯（自動、預設、NULL、固定、序列、範圍、清單、樣式、字典、NULL 比例、seed）、依外鍵順序挑選真實父列、主鍵／唯一值不重複，並以單一交易寫入（每表 10 萬、每次 20 萬列）；自訂字典支援權重、內建字典（姓氏、名字、縣市、英文姓名、訂單狀態）與 CSV 匯入，兩個平台共用檔案格式；Windows 版另有「運算式」規則，可依同一列其他欄位計算（跨欄位條件）。待補 Linux／macOS 運算式規則與 CHECK 約束推論。 |
...
| 協同合作：同步連線、查詢、pipeline、片段、模型、BI、群組 | 🟡 | 第一階段完成（Windows）：「檔案 > 匯出／匯入工作區」把連線（含群組、星號、顏色，不含密碼與本機憑證，帳號可選）、SQL 片段、AI 動作、自動執行作業（Webhook 可選）與資料產生器字典存成排序穩定、不含時間戳記的資料夾，適合 Git 版控；匯入時逐項預覽新增／變更／相同並選擇套用，連線一律合併並保留本機密碼。模型（.punkmodel）與儀表板（.punkbi）本身就是檔案，可直接放進同一個儲存庫。待補可自架同步服務與權限。 |
