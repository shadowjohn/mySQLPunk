...
| 結構同步 | 🟡 | 兩庫唯讀結構差異報告已完成，可跨 provider 比對 Table、Column、PK 與 FK 並匯出 HTML（Linux／macOS 預覽版另含索引與 FK 規則比對）；待補產生／審核 DDL。 |
