...
public static class SmokeTests
{
    ...
    public static int Main()
    {
        ...
        Run("Table Designer 跨資料庫欄位屬性", TestTableDesignerProviderColumnProperties, ref passed);
    ...
    private static void TestTableDesignerProviderColumnProperties()
    {
        string postgreSqlIdentity = BuildProviderColumnDefinition(new my_postgresql(), "main", "public.property_demo", row =>
        {
            row["Name"] = "id";
            row["Type"] = "bigint";
            row["Identity"] = true;
            row["IdentityGeneration"] = "BY DEFAULT";
            row["IdentityOptions"] = "START WITH 10 INCREMENT BY 2";
        });
        AssertContains(postgreSqlIdentity, "GENERATED BY DEFAULT AS IDENTITY (START WITH 10 INCREMENT BY 2)", "PostgreSQL identity options should be included in create SQL.");
        AssertContains(postgreSqlIdentity, "NOT NULL", "PostgreSQL identity should imply NOT NULL.");

        string postgreSqlGenerated = BuildProviderColumnDefinition(new my_postgresql(), "main", "public.property_demo", row =>
        {
            row["Name"] = "name_key";
            row["Type"] = "text";
            row["GeneratedExpression"] = "lower(name)";
            row["GeneratedStorage"] = "STORED";
            row["Collation"] = "public.natural_sort";
            row["StorageMode"] = "EXTENDED";
            row["Compression"] = "lz4";
        });
        AssertContains(postgreSqlGenerated, "STORAGE EXTENDED", "PostgreSQL storage mode should be retained.");
        AssertContains(postgreSqlGenerated, "COMPRESSION \"lz4\"", "PostgreSQL compression should be retained.");
        AssertContains(postgreSqlGenerated, "COLLATE \"public\".\"natural_sort\"", "PostgreSQL collation should quote every identifier part.");
        AssertContains(postgreSqlGenerated, "GENERATED ALWAYS AS (lower(name)) STORED", "PostgreSQL generated expression should be included.");

        string sqliteGenerated = BuildProviderColumnDefinition(new my_sqlite(), "main", "property_demo", row =>
        {
            row["Name"] = "name_key";
            row["Type"] = "TEXT";
            row["GeneratedExpression"] = "lower(name)";
            row["GeneratedStorage"] = "VIRTUAL";
            row["Collation"] = "NOCASE";
        });
        AssertContains(sqliteGenerated, "COLLATE \"NOCASE\"", "SQLite collation should be retained.");
        AssertContains(sqliteGenerated, "GENERATED ALWAYS AS (lower(name)) VIRTUAL", "SQLite generated expression should be included.");

        string oracleIdentity = BuildProviderColumnDefinition(new my_oracle(), "MAIN", "PROPERTY_DEMO", row =>
        {
            row["Name"] = "ID";
            row["Type"] = "NUMBER";
            row["Identity"] = true;
            row["IdentityGeneration"] = "ALWAYS";
            row["IdentityOptions"] = "START WITH 100 INCREMENT BY 5";
        });
        AssertContains(oracleIdentity, "GENERATED ALWAYS AS IDENTITY (START WITH 100 INCREMENT BY 5)", "Oracle identity options should be included in create SQL.");

        string oracleDefaultOnNull = BuildProviderColumnDefinition(new my_oracle(), "MAIN", "PROPERTY_DEMO", row =>
        {
            row["Name"] = "STATE";
            row["Type"] = "VARCHAR2";
            row["Length"] = "16";
            row["Default"] = "active";
            row["DefaultOnNull"] = true;
        });
        AssertContains(oracleDefaultOnNull, "DEFAULT ON NULL 'active'", "Oracle DEFAULT ON NULL should be included in create SQL.");

        string snowflakeVirtual = BuildProviderColumnDefinition(new my_snowflake(), "MAIN", "PUBLIC.PROPERTY_DEMO", row =>
        {
            row["Name"] = "NAME_KEY";
            row["Type"] = "VARCHAR(40)";
            row["GeneratedExpression"] = "UPPER(NAME)";
            row["GeneratedStorage"] = "VIRTUAL";
            row["Collation"] = "en-ci";
            row["Comment"] = "索引用鍵";
        });
        AssertContains(snowflakeVirtual, "AS (UPPER(NAME)) VIRTUAL", "Snowflake virtual expression should be included.");
        AssertContains(snowflakeVirtual, "COLLATE 'en-ci'", "Snowflake collation should use a string literal.");
        AssertContains(snowflakeVirtual, "COMMENT '索引用鍵'", "Snowflake column comment should be inline.");

        DataTable postgreSqlMetadata = LoadProviderDesignerColumnsFromMetadata(new my_postgresql(), new[]
        {
            new object[] { "record_id", "bigint", "NO", "", DBNull.Value, DBNull.Value, DBNull.Value, "bigint", "YES", "BY DEFAULT", "NEVER", "", "", "p", "" }
        }, new[] { "column_name", "data_type", "is_nullable", "column_default", "character_maximum_length", "numeric_precision", "numeric_scale", "ProviderType", "IsIdentity", "IdentityGeneration", "IsGenerated", "GenerationExpression", "Collation", "StorageMode", "Compression" });
        Assert(GetDesignerColumnBool(postgreSqlMetadata.Rows[0], "Identity"), "PostgreSQL identity metadata should load into the designer property model.");
        AssertEquals("BY DEFAULT", Convert.ToString(postgreSqlMetadata.Rows[0]["IdentityGeneration"]), "PostgreSQL identity generation should be retained.");
        AssertEquals("PLAIN", Convert.ToString(postgreSqlMetadata.Rows[0]["StorageMode"]), "PostgreSQL storage code should be normalized for the property panel.");
    }

    private static string BuildProviderColumnDefinition(IDatabase database, string databaseName, string tableName, Action<DataRow> configure)
    {
        using (TableDesignerForm form = new TableDesignerForm(database, databaseName, null))
        {
            ...
            MethodInfo buildMethod = typeof(TableDesignerForm).GetMethod("BuildGenericColumnDefinition", BindingFlags.Instance | BindingFlags.NonPublic);
            return (string)buildMethod.Invoke(form, new object[] { row });
        }
    }

    private static DataTable LoadProviderDesignerColumnsFromMetadata(IDatabase database, object[][] values, string[] columnNames)
    {
        using (TableDesignerForm form = new TableDesignerForm(database, "main", null))
        {
            MethodInfo createTableMethod = typeof(TableDesignerForm).GetMethod("CreateColumnsDisplayTable", BindingFlags.Instance | BindingFlags.NonPublic);
            ...
            foreach (string columnName in columnNames) metadata.Columns.Add(columnName);
            foreach (object[] value in values) metadata.Rows.Add(value);
  ...
  ""sizeBytes"": 16
    ...
    private static void TestRightPaneCollapseBehavior()
    {
        ...
        AssertEquals("0", widthMethod.Invoke(null, new object[] { false, false, false }).ToString(), "Closed right panes should not reserve layout width.");
        AssertEquals("380", widthMethod.Invoke(null, new object[] { true, false, false }).ToString(), "Expanded AI pane should keep its full width.");
        AssertEquals("44", widthMethod.Invoke(null, new object[] { false, true, false }).ToString(), "Collapsed panes should leave only the right rail visible.");
        AssertEquals("424", widthMethod.Invoke(null, new object[] { true, true, false }).ToString(), "Expanded AI pane and collapsed detail pane should share the right edge without overlap.");
        AssertEquals("381", widthMethod.Invoke(null, new object[] { true, false, true }).ToString(), "Expanded object details and AI panes should reserve one pixel for their divider.");
        ...
        AssertContains(formSource, "rightPaneSeparator = new UiDivider", "Object details and AI assistant should use the shared themed divider control.");
        AssertContains(formSource, "aiExpanded && IsInfoPaneExpanded()", "The divider should only appear while both adjacent panes are expanded.");
