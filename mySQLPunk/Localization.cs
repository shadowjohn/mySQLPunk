...
namespace mySQLPunk
{
    public static class Localization
    {
        ...
        static Localization()
        {
            ...
            Add("Redis.ReadOnlyFirstPhase", "Redis 第一階段僅支援唯讀瀏覽與受限查詢；不支援寫入命令。", "The first Redis phase supports read-only browsing and restricted queries; write commands are not supported.");
            Add("Redis.ReadOnlyNote", "第一階段為唯讀：可瀏覽 db 與 keys、檢視型別／TTL／內容摘要，並以 pattern 或單一 key 查詢。使用 RESP2，目標相容 Redis 與 Microsoft Garnet；實機相容矩陣仍待補齊。", "Phase one is read-only: browse dbs and keys, inspect type/TTL/value summaries, and query by pattern or single key. It uses RESP2 and targets Redis and Microsoft Garnet; the live compatibility matrix is still pending.");
